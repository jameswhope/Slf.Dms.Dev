IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'stp_NonDeposit_GetPendingToCallCIDCount')
	BEGIN
		DROP  Procedure  stp_NonDeposit_GetPendingToCallCIDCount
	END

GO

CREATE Procedure stp_NonDeposit_GetPendingToCallCIDCount
AS
Select  Ct=count(distinct m.ClientId)
		FROM 
