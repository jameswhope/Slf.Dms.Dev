...
CREATE Procedure stp_PaymentArrangement_Report
...
Where 
(@PaymentScheduleId is null or ps.PmtScheduleID = @PaymentScheduleId)
and (@Companyid is null or c.companyid = @Companyid)
and (@PaymentDateFrom is null or ps.PmtDate >= @PaymentDateFrom)
and (@PaymentDateTo is null or ps.PmtDate < @PaymentDateTo)
and DATEADD(d,1,ps.PmtDate) >  GetDate() 
and ps.RegisterId is null
