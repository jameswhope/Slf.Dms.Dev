IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'stp_CollectMonthlyFee.sql')
	BEGIN
		DROP  Procedure  stp_CollectMonthlyFee.sql
	END
GO

CREATE Procedure stp_CollectMonthlyFee.sql
	(
...
from 
	tblclient 
where
	isnull(RemittName,'') not like 'Woolery Accountancy%' 
	and TrustID <> 24
	and currentclientstatusid not in (15,17,18) and 
	(
