IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'stp_ChangeSettlementDeliveryMethod')
	BEGIN
		DROP  Procedure  stp_ChangeSettlementDeliveryMethod
	END

GO

CREATE Procedure [dbo].[stp_ChangeSettlementDeliveryMethod]
(	
	@SettlementId int,
...
BEGIN
	...
	SELECT @CheckAmount = (CASE WHEN (@DeliveryMethod = 'P' and @DeliveryAmount <> 15) THEN (@SettlementAmount + @DeliveryAmount)
								  ELSE @SettlementAmount  END)
