IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'stp_settlements_insertSettlementOffer')
	BEGIN
		DROP  Procedure  stp_settlements_insertSettlementOffer
	END

GO

CREATE Procedure stp_settlements_insertSettlementOffer
(
...
@deliverymethod varchar(10) = 'chkbytel',
@deliveryamt money = 0.00,
...
@IsClientStipulation bit = 0,
@IsRestrictiveEndorsement bit = 0,
@UserId int,
@SDABalance money = 0.00,
@BankReserve money = 0.00,
@AvailSDA money = 0.00,
@PFOBalance money = 0.00,
@IsPaymentArrangement bit = 0
)
AS
BEGIN
/*
...
SettlementFee, deliverymethod,deliveryamount,OvernightDeliveryAmount, SettlementCost,SettlementFeeAmtAvailable, SettlementFeeAmtBeingPaid, SettlementFeeAmtStillOwed,SettlementNotes, [Status], Created,CreatedBy, LastModified, LastModifiedBy,SettlementRegisterHoldID, OfferDirection, SettlementSessionGuid,SettlementFeeCredit,Active,IsClientStipulation,
IsRestrictiveEndorsement,SDABalance,BankReserve,AvailSDA,PFOBalance,IsPaymentArrangement)
...
@SettlementAmtStillOwed, @SettlementDueDate, @SettlementSavings,@SettlementFee,@deliverymethod ,@deliveryamt,@OvernightDeliveryAmount, @SettlementCost,@SettlementFeeAmtAvailable, @SettlementFeeAmtBeingPaid, @SettlementFeeAmtStillOwed,
@SettlementNotes, @Status, GetDate(),@UserId, GetDate(), @UserId,@SettlementRegisterHoldID, @OfferDirection, @SettlementSessionGuid,@SettlementFeeCredit,1,@IsClientStipulation,
@IsRestrictiveEndorsement,@SDABalance,@BankReserve,@AvailSDA,@PFOBalance,@IsPaymentArrangement)
...
GRANT EXEC ON stp_settlements_insertSettlementOffer TO PUBLIC
