IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'stp_InsertNoteForSettlementMatter')
	BEGIN
		DROP  Procedure  stp_InsertNoteForSettlementMatter
	END

GO

CREATE Procedure [dbo].[stp_InsertNoteForSettlementMatter]
(
	@ClientId int,
	@MatterId int = null,
...
BEGIN
	...
	BEGIN TRY
		...
		--associate with client, matter and accountId
		...
		IF @CreditorAcctId is not null BEGIN
			INSERT INTO tblNoteRelation(NoteId, RelationTypeId, RelationId)
			VALUES(@NoteId, 2, @CreditorAcctId)
...
GRANT EXEC ON stp_InsertNoteForSettlementMatter TO PUBLIC
