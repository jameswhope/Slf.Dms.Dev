IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'stp_GetPendingSettlementForClient')
	BEGIN
		DROP  Procedure  stp_GetPendingSettlementForClient
	END

GO

CREATE Procedure [dbo].[stp_GetPendingSettlementForClient]
(
@settlementid int
)
...
BEGIN
	...
	WHERE 
		s.settlementid = @settlementid and m.matterstatuscodeid <> 37 and m.isdeleted <> 1 and datediff(d,s.settlementduedate,getdate())<=0
...
GRANT EXEC ON stp_GetPendingSettlementForClient TO PUBLIC
