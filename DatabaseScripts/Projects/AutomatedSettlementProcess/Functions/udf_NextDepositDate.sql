/*IF EXISTS (SELECT * FROM sysobjects WHERE name = 'udf_NextDepositDate')
	DROP  function  udf_NextDepositDate
GO*/

create function [dbo].[udf_NextDepositDate] 
(
	@clientid int
) 
returns datetime
as
begin
-- same as stp_ExpectedDeposits

	declare @enddate datetime, @nextdepositdate datetime
	...
	-- output
	select top 1 @nextdepositdate=depositdate
	from @expected
