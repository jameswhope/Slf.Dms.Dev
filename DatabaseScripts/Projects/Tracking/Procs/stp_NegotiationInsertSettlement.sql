...
--7.29.10.ug.do not set rejected settlements as active
--allows Data entry to reject settlements and not affect 
--the active acceptedsettlement
if @status <> 'R'
	BEGIN
...
@SettlementNotes, @Status, GetDate(),@UserId, GetDate(), @UserId,@SettlementRegisterHoldID, @OfferDirection, @SettlementSessionGuid,@SettlementFeeCredit,@activeflag)
...
if @status <> 'R'
	BEGIN
		...
		tblUser AS neg with(nolock) ON neg.UserID = ne.UserID
