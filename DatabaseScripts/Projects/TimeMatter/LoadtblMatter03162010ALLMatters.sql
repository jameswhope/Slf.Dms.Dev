...
-- Load Matter into tblMatter
-- Total Rows :  80382 
-- Expected   :  80316 (QA) 	 
-- To Dos	  : load AttorneyId
...
, CASE WHEN LEN(mat1_01_04) <>4 THEN CAST(m.memo as varchar) +'--'+mat1_01_04
		ELSE CAST(m.memo as varchar) END as [MatterMemo]
--SUBSTRING(m.memo,0,200) 
,dateadd(s, (m.c_time - 1)/100, dateadd(d, m.c_date, '12-28-1800')) as [CreatedDateTime]
--,m.created_by as [created_by]   -- for checking
...
,CASE WHEN m.archive =1 then 2
	 ...
	 ELSE NULL END)
	 END	 as [MatterStatusId]
----,m.Status as [tmsubtatus]  -- for checking
,CASE WHEN m.archive=1 then 13 -- Note 13 is None
	  ELSE
		(CASE WHEN m.Status IN('No Rep','FDCPA-SA','None','Closed','AALC','AALC Closed','CH','LD w.o/PREJUDICE','LD w/PREJUDICE','LFAC-DWOP','Completed') THEN 13 -- 13 is none
		  WHEN m.Status IN('CHASE-P/A','Consultation','HC Sent to LC','LFAC','PCHC','PCHCR','Pending','Pro Bono','Pro Per') Then 16
		  WHEN m.Status IN('Post Judgment') then 18
...
--- need better queries as duplicate users are here
left join tblUser u2 on (u2.firstname+u2.lastname)= (sf.first+sf.last) and  u2.Locked=0  and u2.System=0 and u2.UserTypeId =1

where 
--m.archive =1
--and  mat1_01_04<> ''
--and mat1_01_04  LIKE '%[^0-9]%'
