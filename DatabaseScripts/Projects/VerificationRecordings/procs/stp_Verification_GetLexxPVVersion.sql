IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'stp_Verification_GetLexxPVVersion')
	BEGIN
		DROP  Procedure  stp_Verification_GetLexxPVVersion
	END

GO

CREATE Procedure stp_Verification_GetLexxPVVersion
@clientid as int
AS
select versionid = case when companyid = 10 then 4 else 3 end from tblclient where clientid = @clientid
