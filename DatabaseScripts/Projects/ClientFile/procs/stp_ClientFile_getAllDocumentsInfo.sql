IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'stp_ClientFile_getAllDocumentsInfo ')
	BEGIN
		DROP  Procedure  stp_ClientFile_getAllDocumentsInfo 
	END

GO

CREATE Procedure stp_ClientFile_getAllDocumentsInfo 
(
@clientid int
)
as 
...
select 
	...
	(
		...
		WHERE     (dr.ClientID = @clientid) AND (dr.DeletedFlag <> 1) and (relationtype in ('account','client'))
	) as docData
	Order by created 
END
