...
ms.MatterStatus as MatterStatusCode, 
m.MatterSubStatusId, 
isnull(t.[description],(select MatterSubStatus from tblmattersubstatus where mattersubstatusid=m.MatterSubStatusId)) as MatterSubStatus,
mt.MatterGroupId
...
left outer join dbo.tblMatterStatus as ms on ms.MatterStatusId = m.MatterStatusId
left outer join dbo.tblCreditor cred on cred.CreditorId = ci.CreditorId
left join tbltask t with(nolock) on t.taskid = m.currenttaskid
where c.ClientId = ' + @ClientId + ' ' + @where + ' and IsNull(m.IsDeleted,0)=0 '
+  @OrderBy )
--order by MatterDate desc,MatterNumber desc ')
