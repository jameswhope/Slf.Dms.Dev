IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'stp_PayExtraFees2Lexxiom')
	BEGIN
		DROP  Procedure  stp_PayExtraFees2Lexxiom
	END

GO

CREATE Procedure stp_PayExtraFees2Lexxiom
...
SET @BalDue = (SELECT TOP(1) BalanceDue FROM tblTMLF2Lexx ORDER BY RecordID DESC)
		...
		BEGIN TRANSACTION tx_ModRecord_nr2
			...
			IF @OriginalAmount > @BalDue
				BEGIN
					...
					INSERT INTO tblTMLF2Lexx (TransactionDate, TransactionType,RefTable,RefTableID,OriginalCommRecID,OriginalAmount,NewAmount,TransactionAmount,BalanceDue,PaidtoCommRecID,CreatedBy)
					...
					UPDATE tblTMLF2Lexx SET BalanceDue = BalanceDue+TransactionAmount where RecordID = @NewTransactionID
					UPDATE tblNachaRegister2 SET Amount = @OriginalAmount+@TransactionAmount, NOTES = 'MODIFIED: SEE tblTMLF2Lexx' WHERE nacharegisterid = @transid
					INSERT INTO tblNachaRegister2 (NachaFileID,CompanyId,Created,flow,commrecid,Name,AccountNumber,RoutingNumber, Notes, Amount, IsPersonal, TrustId, IsAttorney, [Type])
						values (-1,@CompanyId,@RunDate,'debit',@NewCommRecID, @newCommRecName,@newCommRecAcct, @NewCommRecRoute, 'INSERTED BY TMLF2LEXX PROC', abs(@TransactionAmount), 0, @TrustId, 0, @NewCommRecType)
					SET @BalDue = (SELECT TOP(1) BalanceDue FROM tblTMLF2Lexx ORDER BY RECORDID DESC)
				END
			ELSE
				BEGIN -- tblNachaRegister2 amount <= the amount owed to Lexx
					...
					SET @NewTransactionID = scope_identity()
					UPDATE tblTMLF2Lexx SET BalanceDue = BalanceDue+TransactionAmount where RecordID = @NewTransactionID
					UPDATE tblNachaRegister2 SET CommRecID = @newCommRecID, name=@newCommRecName, AccountNumber = @newCommRecAcct, RoutingNumber = @NewCommRecRoute, NOTES = 'MODIFIED: SEE tblTMLF2Lexx' WHERE nacharegisterid = @transid
				END
		...
		BEGIN TRANSACTION tx_ModRecord_nr
			...
			IF @OriginalAmount > @BalDue
				BEGIN
					...
					SET @NewTransactionID = scope_identity()
					UPDATE tblTMLF2Lexx SET BalanceDue = BalanceDue+TransactionAmount where RecordID = @NewTransactionID
					UPDATE tblNachaRegister SET Amount = @OriginalAmount+@TransactionAmount, NOTES = 'MODIFIED: SEE tblTMLF2Lexx' WHERE nacharegisterid = @transid
					INSERT INTO tblNachaRegister (NachaFileID,CompanyId,Created,Name,AccountNumber,RoutingNumber, Notes, Amount, [type], isPersonal,CommRecID, isDeclined, IsAttorney)
						values (NULL,@CompanyId,@RunDate,@newCommRecName,@newCommRecAcct, @NewCommRecRoute, 'INSERTED BY TMLF2LEXX PROC', abs(@TransactionAmount),@NewCommRecType,0,@NRComRecId,0,0)
				END
			ELSE
				BEGIN -- tblNachaRegister amount <= the amount owed to Lexx
					SET @TransactionAmount = @OriginalAmount*-1
					INSERT INTO tblTMLF2Lexx (TransactionDate, TransactionType,RefTable,RefTableID,OriginalCommRecID,OriginalAmount,NewAmount,TransactionAmount,BalanceDue,PaidtoCommRecID,CreatedBy)
					...
					SET @NewTransactionID = scope_identity()
					UPDATE tblTMLF2Lexx SET BalanceDue = BalanceDue+TransactionAmount where RecordID = @NewTransactionID
					UPDATE tblNachaRegister SET name=@newCommRecName, AccountNumber = @newCommRecAcct, RoutingNumber = @NewCommRecRoute, NOTES = 'MODIFIED: SEE tblTMLF2Lexx' WHERE nacharegisterid = @transid
					SET @BalDue = (SELECT TOP(1) BalanceDue FROM tblTMLF2Lexx ORDER BY RECORDID DESC)
				END
