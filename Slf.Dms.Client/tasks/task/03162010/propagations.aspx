...
<head runat="server">
    ...
    <script src="<%= ResolveUrl("~/jscript/onlydigits.js")%>"></script>
</head>
<body onload="LoadPropagations();">
    ...
    function LoadPropagations()
    {
        ...
        if (value.length > 0)
        {
            ...
            for (a = 0; a < Propagations.length; a++)
            {
                ...
                AddPropagation(IsResolved, AssignedTo, DueType, Date, Due, TaskType, Description);
            }
        }
    ...
    function AddPropagationClick(lnk)
    {
        if (!lnk.disabled)
        {
            AddPropagation(false, 0, 0, "", "", 0, "");
        }
    }
    function AddPropagation(IsResolved, AssignedTo, DueType, Date, Due, TaskType, Description)
    {
        var tblTaskPropagation = document.getElementById("<%= tblTaskPropagation.ClientID %>");
        ...
        var cboAssignedTo = GetNewAssignedTo(AssignedTo);
    ...
    function ResetPropagations()
    {
        ...
        for (i = 1; i < tblTaskPropagation.rows.length; i += 2)
        {
            ...
            var cboDueType = tblTaskPropagation.rows[i].cells[2].childNodes[0];
            var txtDueDate = tblTaskPropagation.rows[i].cells[3].childNodes[0];
            var txtDue = tblTaskPropagation.rows[i].cells[3].childNodes[1];
            ...
            txtPropagations.value += cboAssignedTo.options[cboAssignedTo.selectedIndex].value + "," + cboDueType.options[cboDueType.selectedIndex].value + "," + txtDueDate.value + "," + txtDue.value + "," + cboTaskType.options[cboTaskType.selectedIndex].value + "," + txtDescription.value;
	...
	function RequiredExist()
	{
        ...
        for (i = 1; i < tblTaskPropagation.rows.length; i += 2)
        {
            ...
            // Due Date or Days Due
            if (cboDueType.options[cboDueType.selectedIndex].value == 0) // 0 - Date
            {
	            ...
	            else if (!RegexValidate(txtDueDate.value, "(?=\\d)^(?:(?!(?:10\\D(?:0?[5-9]|1[0-4])\\D(?:1582))|(?:0?9\\D(?:0?[3-9]|1[0-3])\\D(?:1752)))((?:0?[13578]|1[02])|(?:0?[469]|11)(?!\\/31)(?!-31)(?!\\.31)|(?:0?2(?=.?(?:(?:29.(?!000[04]|(?:(?:1[^0-6]|[2468][^048]|[3579][^26])00))(?:(?:(?:\\d\\d)(?:[02468][048]|[13579][26])(?!\\x20BC))|(?:00(?:42|3[0369]|2[147]|1[258]|09)\\x20BC))))))|(?:0?2(?=.(?:(?:\\d\\D)|(?:[01]\\d)|(?:2[0-8])))))([-.\\/])(0?[1-9]|[12]\\d|3[01])\\2(?!0000)((?=(?:00(?:4[0-5]|[0-3]?\\d)\\x20BC)|(?:\\d{4}(?!\\x20BC)))\\d{4}(?:\\x20BC)?)(?:$|(?=\\x20\\d)\\x20))?((?:(?:0?[1-9]|1[012])(?::[0-5]\\d){0,2}(?:\\x20[aApP][mM]))|(?:[01]\\d|2[0-3])(?::[0-5]\\d){1,2})?$"))
	            {
	                ShowMessage("The Due Date you entered for Propagation " + ((i + 1) / 2) + " is invalid.  Please enter a date/time value.");
	                AddBorder(txtDueDate);
	                return false;
	            }
	            else
            ...
            else // 1 - Days
            {
	            ...
	            if (txtDue.value.length == 0)
	            {
	                ShowMessage("The Days Due From Now is a required field");
	                AddBorder(txtDue);
	                return false;
	            }
                else if (isNaN(parseFloat(txtDue.value)) || parseFloat(txtDue.value) < 0)
	            {
	                ShowMessage("The Days Due From Now you entered for Propagation " + ((i + 1) / 2) + " is invalid.  Please enter a decimal value.");
	                AddBorder(txtDue);
	                return false;
	            }
	            else
            ...
            <tr>
                <td valign="top" style="padding-left:10;height:100%;">
                    ...
                    <table style="font-family:tahoma;font-size:11px;width:100%;" border="0" cellpadding="0" cellspacing="0">
                        <tr><td class="cLEnrollHeader">Task Propagation</td></tr>
                        <tr>
                                ...
                                <table runat="server" id="tblPropagation" style="width:100%;font-family: tahoma; font-size: 11px;" cellpadding="0" cellspacing="0" border="0">
                                        ...
                                        <td>
                                            <table id="tblTaskPropagation" runat="server" style="margin-bottom:10;font-family:tahoma;font-size:11px;width:100%;" border="0" cellpadding="5" cellspacing="0">
                                                <tr>
                                                    ...
                                                    <td class="headItem2" style="width:75;">Assigned To</td>
                                                    <td class="headItem2" style="width:60;">When</td>
                                                    <td class="headItem2" style="width:65;">Due</td>
                                                    <td class="headItem2">Task Type</td>
                                                </tr>
                                            </table>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td style="padding-left:5;">
                                            <a runat="server" id="lnkAddPropagation" href="#" onclick="AddPropagationClick(this);return false;" class="lnk" style="color: rgb(0,0,159);">Add New Task</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a runat="server" href="#" onclick="ClearPropagations(this);return false" id="lnkClearPropagations" class="lnk" style="color: rgb(0,0,159);">Clear All</a>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                ...
                <td style="height:40px;border-top:solid 2px rgb(149,180,234);padding-left:10px;padding-right:10px;">
                    <table style="height:100%;font-family:tahoma;font-size:11px;width:100%;" border="0" cellpadding="0" cellspacing="0">
                        <tr>
                            ...
                            <td align="right"><a tabindex="4" style="color:black" class="lnk" href="javascript:SavePropagations();">Save Task Propagations<img style="margin-left:6px;" runat="server" src="~/images/16x16_forward.png" border="0" align="absmiddle"/></a></td>
                        </tr>
                    </table>
                </td>
            </tr>
