...
Partial Class Agency_Potential_Deposits
    ...
    Private Sub LoadGrid()
        ...
        Using cmd As SqlCommand = ConnectionFactory.CreateCommand("stp_ProjectedDeposits")
            Using cmd.Connection
                ...
                tblAgency.Columns.Add("Agency")

                tblCompany.Columns.Add("Agency")
                tblCompany.Columns.Add("Company")

                tblProjections.Columns.Add("Agency")
                ...
                For Each row As DataRow In dsResults.Tables(0).Rows
                    tblAgency.Columns.Add(Format(row(0), "M/d/yy"), Type.GetType("System.Decimal"))
                    tblCompany.Columns.Add(Format(row(0), "M/d/yy"), Type.GetType("System.Decimal"))
                    tblProjections.Columns.Add(Format(row(0), "M/d/yy"), Type.GetType("System.Decimal"))
                Next

                'add amounts into each date column
                For Each agency As DataRow In dsResults.Tables(1).Rows
                    agencys = dsResults.Tables(3).Select("agency = '" & agency(0) & "'")

                    newAgencyRow = tblAgency.NewRow
                    newAgencyRow("Agency") = agency(0)
                    For Each daterow In agencys
                        newAgencyRow(Format(daterow("date"), "M/d/yy")) = daterow("amount")
                    Next
                    tblAgency.Rows.Add(newAgencyRow)

                    For Each company As DataRow In dsResults.Tables(2).Rows
                        rows = dsResults.Tables(4).Select("agency = '" & agency(0) & "' and company = '" & company(0) & "'")

                        If rows.Length > 0 Then
                            newCompanyRow = tblCompany.NewRow
                            newCompanyRow("Agency") = agency(0)
                            newCompanyRow("Company") = company(0)

                            For Each daterow In rows
                                newCompanyRow(Format(daterow("date"), "M/d/yy")) = daterow("amount")
                            Next

                            tblCompany.Rows.Add(newCompanyRow)

                            'get projections for this company, agency
                            projections = dsResults.Tables(5).Select("agency = '" & agency(0) & "' and company = '" & company(0) & "'")
                            ...
                            For Each projection As DataRow In projections
                                If Format(projection("projectedon"), "M/d/yy") <> curProjOn Then
                                    ...
                                    newProjRow("Agency") = projection("Agency")
                                    newProjRow("Company") = projection("Company")
                                    newProjRow("ProjectedOn") = Format(projection("projectedon"), "MM/dd/yyyy")
                                End If
                                If tblProjections.Columns.Contains(Format(projection("fordate"), "M/d/yy")) Then
                                    newProjRow(Format(projection("fordate"), "M/d/yy")) = projection("amount")
                                End If
                                If Format(projection("projectedon"), "M/d/yy") <> curProjOn Then
                                    tblProjections.Rows.Add(newProjRow)
                                    curProjOn = Format(projection("projectedon"), "M/d/yy")
                                End If
                            Next
                        End If
                    Next 'company
                ...
                dsOutput.Tables.Add(tblAgency)
                dsOutput.Tables.Add(tblCompany)
                dsOutput.Tables.Add(tblProjections)

                dsOutput.Relations.Add("AgencyCompany", dsOutput.Tables("Agency").Columns("Agency"), dsOutput.Tables("Company").Columns("Agency"))

                parentCols(0) = dsOutput.Tables("Company").Columns("Agency")
                parentCols(1) = dsOutput.Tables("Company").Columns("Company")

                childCols(0) = dsOutput.Tables("Projections").Columns("Agency")
                childCols(1) = dsOutput.Tables("Projections").Columns("Company")

                dsOutput.Relations.Add("CompanyProjections", parentCols, childCols)

                UltraWebGrid1.DataSource = tblAgency
    ...
    Protected Sub UltraWebGrid1_InitializeLayout(ByVal sender As Object, ByVal e As Infragistics.WebUI.UltraWebGrid.LayoutEventArgs) Handles UltraWebGrid1.InitializeLayout
        With UltraWebGrid1
            ...
            .Bands(0).Columns(0).Header.Caption = "Agency"
