...
Partial Class research_queries_financial_servicefees_byagency
...
#Region "Event"
    ...
    Private Sub Save()
        ...
        QuerySettingHelper.Insert(Me.GetType().Name, UserID, csCommScenId.ID, "store", csCommScenId.SelectedStr)
...
#Region "Util"
    Private Sub LoadEntryTypes()
        ...
        End If
    End Sub
    Private Sub LoadCommScenIds()
        Dim s As New List(Of String)
        If csCommScenId.SelectedList.Count > 0 Then
            For Each str As String In csCommScenId.SelectedList
                s.Add(str)
            Next
        Else
            For Each str As String In csCommScenId.EntireList
                s.Add(str)
            Next
        End If
        CommScenIds = String.Join(",", s.ToArray())

        If String.IsNullOrEmpty(CommScenIds) Then
            CommScenIds = "-1"
        End If
    ...
    End Sub
    Private Sub LoadClients(ByVal IsPostBack As Boolean)
        ...
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            Using cmd.Connection
                cmd.CommandText = "SELECT tblClient.ClientId, tblPerson.FirstName, tblPerson.LastName FROM tblClient INNER JOIN tblPerson ON tblPerson.PersonId=tblClient.PrimaryPersonId WHERE AgencyId IN (SELECT AgencyId FROM tblCommScen WHERE CommScenId IN (" & AllCommScenIds & ")) ORDER BY tblPerson.LastName"
                cmd.Connection.Open()
    ...
    Private Sub LoadAgencies()
        csCommScenId.Items.Clear()
        csCommScenId.AddItem(New ListItem(" -- Select --", 0))

        Dim lstAllCommScenIds As New List(Of String)
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            Using cmd.Connection
                cmd.CommandText = "SELECT tblAgency.Name, tblCommScen.CommScenId FROM tblCommScen INNER JOIN tblAgency ON tblCommScen.AgencyId=tblAgency.AgencyId"
                ...
                Using rd As IDataReader = cmd.ExecuteReader()
                    While rd.Read()
                        ...
                        csCommScenId.AddItem(New ListItem(AgencyName, iCommScenId))
                    End While
                End Using
            End Using
        End Using
        ...
        End If
        If Not IsPostBack Then
            csCommScenId.SelectedStr = DataHelper.FieldLookup("tblQuerySetting", "Value", _
                "UserID = " & UserID & " AND ClassName = '" & Me.GetType().Name & "' AND [Object] = '" & csCommScenId.ID + "'")
        End If
    End Sub
    ...
    Public Overrides Sub AddPermissionControls(ByVal c As System.Collections.Generic.Dictionary(Of String, System.Web.UI.Control))
        AddControl(pnlBody, c, "Research-Queries-Financial-Service Fees-Service Fees By Agency")
    End Sub
#End Region
#Region "Query"
    ...
    Private Sub AddStdParams(ByVal cmd As IDbCommand)
        ...
        DatabaseHelper.AddParameter(cmd, "orderby", "tblagency.agencyid")
        DatabaseHelper.AddParameter(cmd, "CommRecId", CommRecId)
        DatabaseHelper.AddParameter(cmd, "CommScenIds", CommScenIds)
    End Sub
    Private Sub Requery(ByVal Reset As Boolean)
        ...
        Dim cmd As IDbCommand = ConnectionFactory.CreateCommand("stp_QueryGetServiceFeePayments")
        AddStdParams(cmd)
        cmd.CommandTimeout = 180
        grd.DataCommand = cmd

        Session("rptcmd_query_servicefee_byagency_payments") = cmd
