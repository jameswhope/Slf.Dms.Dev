...
#Region "Event"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            Using cmd.Connection
                cmd.CommandText = "SELECT MatterID FROM tblMatter Where IsNull(IsDeleted,0)=0"
                cmd.Connection.Open()
    ...
    Protected Sub lnkExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkExport.Click
        ...
        strQuery = "Select T.TaskID, M.MatterID, T.Created as CreatedDate, T.Due as DueDate, T.TaskResolutionID, " & _
     ...
     " (Select FirstName+' '+ LastName from tbluser Where UserID=T.CreatedBy) as CreatedBy, T.Resolved, R.MatterTypeId" & _
     " FROM dbo.tblMatter R, dbo.tblMatterTask M, dbo.tblTask T  Where (T.TaskID = M.TaskID AND M.MatterID=R.MatterID and IsNull(R.IsDeleted,0)=0) "
...
#Region "Util"
    ...
    Private Sub LoadMatterTasks()
        ...
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            Using cmd.Connection
                cmd.CommandText = "SELECT MatterId, MatterMemo as Name FROM tblMatter Where IsNull(IsDeleted,0)=0 ORDER BY MatterId"
                cmd.Connection.Open()
                ...
                Using rd As IDataReader = cmd.ExecuteReader()
                    While rd.Read()
                        ...
                        csMatterNoteID.AddItem(New ListItem(MatterID, MatterID))
    ...
    Private Sub PopulateMatterTypes()
        csMatterType.Items.Clear()
        csMatterType.AddItem(New ListItem(" -- Select --", 0))
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            Using cmd.Connection
                ...
                Using rd As IDataReader = cmd.ExecuteReader()
                    While rd.Read()
                        Dim MatterTypeID As Integer = DatabaseHelper.Peel_int(rd, "MatterTypeId")
                        Dim Name As String = DatabaseHelper.Peel_string(rd, "MatterTypeCode")
                        csMatterType.AddItem(New ListItem(Name, MatterTypeID))
        ...
        End Using
        If Not IsPostBack Then
            csMatterType.SelectedStr = DataHelper.FieldLookup("tblQuerySetting", "Value", _
                "UserID = " & UserID & " AND ClassName = '" & Me.GetType().Name & "' AND [Object] = '" & csMatterType.ID + "'")
        End If
...
#Region "Query"
    ...
    Private Function GetData() As DataView
        ...
        itemCount = 0
        If optMatterTypeChoice.Items(0).Selected Then
            For itemCount = 0 To csMatterType.SelectedList.Count - 1
                If strExp3 = String.Empty Then
                    strExp3 = " MatterTypeId = " & csMatterType.SelectedList.Item(itemCount)
                Else
                    strExp3 = strExp3 & " and  MatterTypeId = " & csMatterType.SelectedList.Item(itemCount)
                End If
            Next itemCount
        ElseIf optMatterTypeChoice.Items(1).Selected Then
            For itemCount = 0 To csMatterType.SelectedList.Count - 1
                If strExp3 = String.Empty Then
                    strExp3 = " MatterTypeId = " & csMatterType.SelectedList.Item(itemCount)
                Else
                    strExp3 = strExp3 & " or  MatterTypeId = " & csMatterType.SelectedList.Item(itemCount)
                End If
            Next itemCount
        End If
        If strExp3 <> String.Empty Then
    ...
    Protected Sub gvResults_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvResults.RowDataBound
        ...
        ElseIf e.Row.RowType = DataControlRowType.DataRow Then
            If e.Row.Cells(7).Text.Trim() <> "&nbsp;" Then
                ...
                If Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "TaskResolutionId")) = 1 Then
                    e.Row.Cells(7).Text = CType(e.Row.Cells(7).Text, DateTime).ToString("dd MMM, yyyy hh:mm:ss tt")
                    e.Row.Cells(8).Text = "RESOLVED"
                Else
                    e.Row.Cells(8).Text = "IN PROGRESS"
                End If

            ElseIf CType(e.Row.Cells(6).Text, DateTime) < Now() Then
                e.Row.Cells(8).Text = "PAST DUE"
            Else
                e.Row.Cells(8).Text = "OPEN"
            End If
            If e.Row.Cells(5).Text.Trim() <> "&nbsp;" And e.Row.Cells(7).Text.Trim() <> "&nbsp;" Then

                Dim Dur As TimeSpan
                Dur = CType(e.Row.Cells(7).Text, DateTime) - CType(e.Row.Cells(5).Text, DateTime)

                'Return Math.Round(Dur.TotalHours, 2).ToString()
                e.Row.Cells(9).Text = Dur.Days & "d:" & Dur.Hours & "h:" & Dur.Minutes & "m:" & Dur.Seconds & "s"
