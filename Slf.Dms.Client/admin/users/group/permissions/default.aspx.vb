...
Partial Class admin_users_group_permissions_default
...
#End Region
    ...
    Private Sub Requery()
        ...
        Using cmd As IDbCommand = ConnectionFactory.CreateCommand("stp_Permissions_GroupFunctions_Get")
    ...
    Private Sub Save()
        ...
        For Each Permission As String In Permissions
            ...
            Using cmd As IDbCommand = ConnectionFactory.CreateCommand("stp_Permissions_Group_IoU")
                Using cmd.Connection
                    DatabaseHelper.AddParameter(cmd, "UserGroupId", UserGroupId)
    ...
    Protected Sub lnkClearPermissions_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkClearPermissions.Click
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            Using cmd.Connection
                ...
                cmd.CommandText = "DELETE FROM tblPermission WHERE PermissionId IN (SELECT PermissionId FROM tblGroupPermission WHERE UserGroupId=@UserGroupId)"
                cmd.ExecuteNonQuery()

                'delete from tblGroupPermission
                cmd.CommandText = "DELETE FROM tblGroupPermission WHERE UserGroupId=@UserGroupId"
                cmd.ExecuteNonQuery()
            End Using
        End Using
        Requery()
