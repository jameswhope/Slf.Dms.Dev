...
Partial Class admin_users_user_permissions_default
...
#Region "Variables"
    ...
    Protected RecordUserID As Integer
...
#End Region
    ...
    Private Sub Requery()
        ...
        Using cmd As IDbCommand = ConnectionFactory.CreateCommand("stp_Permissions_UserFunctions_Get")
    ...
    Private Sub Save()
        ...
        For Each Permission As String In Permissions
            ...
            Using cmd As IDbCommand = ConnectionFactory.CreateCommand("stp_Permissions_User_IoU")
                Using cmd.Connection
                    DatabaseHelper.AddParameter(cmd, "UserId", RecordUserID)
    ...
    Protected Sub lnkClearPermissions_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkClearPermissions.Click
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            Using cmd.Connection
                DatabaseHelper.AddParameter(cmd, "UserId", RecordUserID)
                cmd.Connection.Open()
                ...
                cmd.CommandText = "DELETE FROM tblPermission WHERE PermissionId IN (SELECT PermissionId FROM tblUserPermission WHERE UserId=@UserId)"
                cmd.ExecuteNonQuery()

                'delete from tblUserPermission
                cmd.CommandText = "DELETE FROM tblUserPermission WHERE UserId=@UserId"
                cmd.ExecuteNonQuery()
            End Using
        End Using
        Requery()
    ...
    Public Overrides Sub AddPermissionControls(ByVal c As System.Collections.Generic.Dictionary(Of String, System.Web.UI.Control))
        AddControl(pnlBody, c, "Users-User Single Record-Permissions")
    End Sub
End Class