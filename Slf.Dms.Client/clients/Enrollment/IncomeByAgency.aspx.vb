...
#Region "Private Routines"
    ...
    Private Sub LoadLeads(ByVal gvLeads As GridView, ByVal FromDate As String, ByVal ToDate As String)
        Try
            ...
            Dim procname As String = "stp_GetIncomeFromAgencies_TEST"
            ...
            If ddlAgency.SelectedValue = -1 Then
                params2.Add(New SqlParameter("commrecids", hdnAgencies.Value))
            Else
                params2.Add(New SqlParameter("commrecids", ddlAgency.SelectedValue))
            End If

            If ddlUser.SelectedValue = -1 Then
                params2.Add(New SqlParameter("users", hdnUsers.Value))
            Else
            ...
            For i As Integer = 0 To ds.Tables(1).Rows.Count
                dt2.Columns.Add()
            Next
            For i As Integer = 0 To ds.Tables(1).Columns.Count - 1
                dt2.Rows.Add()
                dt2.Rows(i)(0) = ds.Tables(1).Columns(i).ColumnName
            Next
            For i As Integer = 0 To ds.Tables(1).Columns.Count - 1
                For j As Integer = 0 To ds.Tables(1).Rows.Count - 1
                    If j = 1 Then
                        dt2.Rows(i)(j + 1) = ds.Tables(1).Rows(j)(i)
                    Else
                        dt2.Rows(i)(j + 1) = CDbl(ds.Tables(1).Rows(j)(i)).ToString("$###,###,###,##0.00")
                    End If

                Next
            Next

            gvTotals.DataSource = dt2
            gvTotals.DataBind()
