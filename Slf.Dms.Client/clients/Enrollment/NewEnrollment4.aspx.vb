...
Partial Class Clients_Enrollment_NewEnrollment4
...
#Region "Methods"
    ...
    Protected Sub Page_LoadComplete(sender As Object, e As System.EventArgs) Handles Me.LoadComplete
        ...
        If Not (UserID = 310 OrElse UserID = 311 OrElse UserID = 1848 OrElse UserID = 2200) Then
            tabGlobalClients.Visible = False
        End If
        If (ddlCompany.SelectedValue = "20") Then
            Response.Redirect(String.Format("~/clients/enrollment/newenrollment2.aspx?id={0}&p1=0&p2=0&s=1", aID))
        End If

        CheckInitialDepositMinimum()
    ...
    Protected Sub btnCreditorRefresh_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCreditorRefresh.Click
        ...
        'dsCreditors.SelectParameters("Represented").DefaultValue = 1


        wGrdCreditors.DataBind()
        wGrdCreditors.Visible = True

        'Pop Initial Deposit Amount
        CheckAutoFillInitialDeposit()
    End Sub

    Protected Sub CheckAutoFillInitialDeposit()
        Try
            Dim params As New List(Of SqlParameter)
            Dim sum As Double = 0
            params.Add(New SqlParameter("@LeadApplicantId", aID))

            Dim result As Object = SqlHelper.ExecuteScalar(String.Format("select isnull(sum(Balance),0) from tblLeadCreditorInstance where leadapplicantid = {0}", aID), CommandType.Text)
            If IsNumeric(result) Then
                sum = CDbl(result)
                If sum > 10000 Then
                    Dim initialdep As Double = 0

                    Dim result2 As Object = SqlHelper.ExecuteScalar(String.Format("select isnull(initialdeposit,0) from tblleadcalculator where leadapplicantid = {0}", aID), CommandType.Text)
                    If IsNumeric(result2) Then
                        initialdep = CDbl(result2)
                        If (initialdep < (sum * 0.035)) Then
                            SqlHelper.ExecuteNonQuery(String.Format("update tblleadcalculator set initialdeposit = {0} where leadapplicantid = {1}", sum * 0.038, aID), CommandType.Text)
        ...
        Catch ex As Exception
            ...
            divMsg.InnerText &= "Unable to set initial deposit to 3.8%. " & ex.Message
        End Try
    End Sub

    Protected Sub CheckInitialDepositMinimum()
        Try
            Dim sum As Double = 0
                ...
                If sum > 10000 Then
                    ...
                    If IsNumeric(result2) Then
                        ...
                        If (initialdep < (sum * 0.035)) Then
                            ...
                            divMsg.InnerHtml &= String.Format("Initial deposit set lower than the minimum of 3.5%. Should be at least ${0}. ", sum * 0.035)
                        End If
                    End If
                End If
            End If
        Catch ex As Exception
            divMsg.Style("display") = ""
            divMsg.InnerText &= "Unable to check initial deposit to 3.8%. " & ex.Message
        End Try
    End Sub
    ...
    Private Sub AssignHardshipData(ByVal rdr As SqlDataReader)
        ...
        If rdr.Read Then
            ...
            txtHardshipOther.Text = rdr("HardshipOther").ToString
    ...
    Private Sub BindGrids()
        ...
        dsCoApp.SelectParameters("applicantID").DefaultValue = aID
        'dsCoApp.SelectParameters("Represented").DefaultValue = 1
        dsCoApp.DataBind()
        ShowGrid(dsCoApp, wGrdCoApp)
    ...
    Private Sub LoadData()
        ...
        If aID > 0 Then
            ...
            Else
                bindDDL(ddlCompany, String.Format("select distinct x.companyid, c.Name from tblStatesAgencyXRef x join tblState s on s.StateID = x.StateId join tblCompany c on c.CompanyId = x.CompanyId where x.AgencyId = {0} and Active = 1", AgencyID), "name", "Companyid", False)
            End If
        Else
            bindDDL(ddlCompany, String.Format("select distinct x.companyid, c.Name from tblStatesAgencyXRef x join tblState s on s.StateID = x.StateId join tblCompany c on c.CompanyId = x.CompanyId where x.AgencyId = {0}  and Active = 1", AgencyID), "name", "Companyid", False)
        End If
    ...
    Private Sub Save(ByVal a As Boolean, ByVal aID As Integer, Optional ByVal bClose As Boolean = False)
        ...
        Else
            ...
            Else
                urlredirect = String.Format("newenrollment3.aspx?callidkey={0}&ani={1}", hdnCallIdKey.Value, hdnAni.Value)
            End If
        End If
    ...
    Private Sub SaveHardshipInfo(ByVal leadApplicantID As Integer)
        ...
        If DataHelper.RecordExists("tblLeadHardship", String.Format("LeadApplicantID={0}", leadApplicantID)) Then
            ...
            sql.AppendFormat("OtherDebts='{0}',", txtOtherDebts.Text.Replace("'", "''"))
        ...
        Else
            ...
            sql.AppendFormat("'{0}',", txtOtherDebts.Text.Replace("'", "''"))
    ...
    Private Sub generateLSA(Optional ByVal bElectronic As Boolean = False, Optional ByVal bUseLexxEsign As Boolean = False)
            ...
            Select Case bUseLexxEsign
                Case True
                    ...
                    For Each doc As LetterTemplates.BatchTemplate In nosign
                        ...
                        'Dim path As String = "D:\lsasigning\tmp\temp" & String.Format("\{0}.html", documentId)
                        ...
                        If Not doc.NeedSignature Then
                            ...
                            'path = "D:\lsasigning\tmp\temp" & String.Format("\{0}.pdf", documentId)
                        ...
                        'If Not doc.NeedSignature Then
                        '    'path = ConfigurationManager.AppSettings("LeadDocumentsDir") & String.Format("\{0}.pdf", documentId)
                        '    Using finalPDF As New GrapeCity.ActiveReports.Export.Pdf.Section.PdfExport
                        '        Using strmPDF As New System.IO.MemoryStream()
                        '            finalPDF.Export(doc.TemplateRpt.Document, strmPDF)
                        '            AzureStorageHelper.ExportLeadDocumentTemp(strmPDF, documentId + ".pdf")
                        '        End Using
                        '        'finalPDF.Export(doc.TemplateRpt.Document, path)
                        '    End Using
                        'End If
                    ...
                    For Each doc As LetterTemplates.BatchTemplate In needsign
                        ...
                        If docIDs.Contains(templateName) Then
                            ...
                            'Dim path As String = "D:\lsasigning\tmp\" & String.Format("temp\{0}.pdf", docIDs(templateName))
                            Using finalPDF As New GrapeCity.ActiveReports.Export.Pdf.Section.PdfExport
                    ...
                    SmartDebtorHelper.SendEsignNotification(txtEmailAddress.Text, dNames.ToArray, signingBatchId, UserID, bSpanish)
        ...
        Else
            Dim rDoc As GrapeCity.ActiveReports.Document.SectionDocument = rpt.Generate_SM_LSA_Mossler_20150427(CompanyID:=CompanyID, StateID:=StateID, ContingencyFeePercent:=ContingencyFeePercent,
              MaintenanceFeeCap:=MaintFeeCap, MaintenanceFeePerAccount:=MaintFeePerAcct, BankName:=BankName,
              BankRoutingNum:=BankRoutingNum, BankAcctNum:=BankAcctNum, InitialDepositAmount:=InitialDepositAmount,
              DepositDay:=DepositDay, DepositCommitmentAmount:=FormatCurrency(DepositCommitmentAmount, 2),
              TypeOfAccount:=typeOfAccount, ClientFirstName:=txtFirstName.Text, ClientLastName:=txtLastName.Text,
        ...
        If Loading = False Then
            Select Case e.Button.Text
                Case "Home"
                    ...
                    End If
                Case "New Applicant"
                    Response.Redirect(String.Format("NewEnrollment3.aspx?p1={0}&p2={1}&s={2}", Request.QueryString("p1"), Request.QueryString("p2"), HttpUtility.UrlEncode(Request.QueryString("s"))))
                Case "Generate LSA"
                    generateLSA()
                Case "Generate e-LSA"
                    generateLSA(True)
    ...
    Public Sub CreatePrivUser() 'CHOLT 7/24/2020
        Try
            ...
            Dim password As String = "Pr1v1ca456"
            Dim confirmPassword As String = "Pr1v1ca456"
            ...
            If Not IsDBNull(dt.Rows(0)("PrivicaId")) Then
                PrivUserID = dt.Rows(0)("PrivicaId")
            Else
                PrivUserID = Nothing
            End If

            'if user has an account, display error and exit sub
            If Not String.IsNullOrEmpty(PrivUserID) Then
                divMsg.Style("display") = ""
                divMsg.InnerText = "User already has an account"
                Exit Sub
            End If
            ...
            'address2 = DataHelper.FieldTop1("tblleadapplicant", "Street2", "LeadApplicantID = " & aID)

            'creates object
            Dim privUser As New PrivicaResults

            'creates user and assigns results to object
            privUser = PrivicaHelper.CreateUser(debug, email, password, confirmPassword)
            ...
            sql.AppendFormat("PrivicaId={0} ", PrivUserID)
            ...
            PrivicaHelper.UpdateUser(debug, PrivUserID, firstName, lastName, city, state, SSN, dateString, address, "", zip)

            'sleep for 2 seconds to catch response
            Threading.Thread.Sleep(2000)

            'creates questions
            privUser = PrivicaHelper.CreateQuestion(debug, PrivUserID)
    ...
    Public Sub PrivQASubmit2() 'cholt 7/24/2020
        Try
            ...
            'import current user's ID
            PrivUserID = DataHelper.FieldTop1("tblLeadApplicant", "PrivicaId", "LeadApplicantID = " & aID)
            ...
            PrivicaHelper.AnswerQuestions(debug, PrivUserID, PrivQ1A, privQ2A, privQ3A, privQ4A, privQ5A)

            'sleep for 2 seconds to catch response
            Threading.Thread.Sleep(2000)

            'After answering questions, submits for review by AET
            PrivicaHelper.RequestReview(debug, PrivUserID)
