...
Partial Class Clients_Enrollment_DepositAnalysisSnapshot
...
#Region "Page Event Handlers"
    ...
    Protected Sub lvLeads_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ListViewItemEventArgs) Handles lvLeads.ItemDataBound
        Dim sel As FilterSelection1 = DirectCast(DirectCast(e.Item, ListViewDataItem).DataItem, FilterSelection1)
...
#Region "Private Routines"
    ...
    Private Sub LoadReport()
        Dim l As New List(Of FilterSelection1)
        If ddlQuickPickDate.SelectedItem.Text.ToLower = "by month" Then
            ...
            For i As Integer = 0 To months
                ...
                l.Add(New FilterSelection1() With {.FilterName = String.Format("{0: MMMM yyyy}", CDate(fromdate)), .FromDate = fromdate, .ToDate = todate})
                fromdate = fromdate.AddMonths(-1)
            Next
        Else
            l.Add(New FilterSelection1() With {.FilterName = BuildFilterName(), .FromDate = CDate(txtDate1.Text), .ToDate = txtDate2.Text})
        End If
...
#End Region
    ...
    Protected Sub ddlMonthYear1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlMonthYear1.SelectedIndexChanged
        SetCompanies()
        SetProducts()
        SetReps()
    End Sub
