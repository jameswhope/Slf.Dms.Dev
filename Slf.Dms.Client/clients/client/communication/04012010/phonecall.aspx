...
<asp:Content ID="cphBody" ContentPlaceHolderID="cphBody" runat="Server"><asp:placeholder id="phBody" runat="server">
    ...
    {
        showModalDialog("<%= ResolveUrl("~/util/pop/addrelation.aspx?to=phonecall&toid=" & PhoneCallID & "&dependencyid=" & ClientID) %>", window, "status:off;help:off;dialogWidth:450px;dialogHeight:350px;");
    }
    ...
    <script type="text/javascript" language="javascript">
        ...
        function OpenScanning()
        {
            ...
            scanWin = window.open('<%=ResolveUrl("~/clients/client/scanning.aspx") %>?id=<%=ClientID %>&type=phonecall&rel=' + relID, 'ScanDocument', 'left=0,top=0,width=' + screen.width + ',height=' + screen.height);
                                        ...
                                        <td colspan="2">
                                            <table onmouseover="Grid_RowHover(this, true)" onmouseout="Grid_RowHover(this,false)" class="list" style="font-family:tahoma;font-size:11px;width:100%;" cellspacing="0" cellpadding="3">
                                                ...
                                                <asp:repeater runat="server" id="rpRelations">
                                                    <itemtemplate>
                                                        <a href="<%#  ResolveURL(ctype(DataBinder.Eval(Container.DataItem, "NavigateURL"),string).replace("$clientid$",ClientID).replace("$aid$",AccountID).replace("$ciid$",CreditorInstanceId).replace("$x$",DataBinder.Eval(Container.DataItem, "RelationID")))  %>"
                                                        <tr>
