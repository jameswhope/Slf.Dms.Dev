...
#End Region
    ...
    Private Sub LoadHardship()
        Dim obj As Object = SqlHelper.ExecuteScalar("select hardshipdate from tblhardshipdata where clientid = " & ClientID, CommandType.Text)
        If IsDate(obj) Then
            ...
            Using cmd.Connection
                ...
                Using rd As IDataReader = cmd.ExecuteReader(CommandBehavior.SingleRow)
                    ...
                    If rd.Read Then
                        ...
                        If DatabaseHelper.Peel_double(rd, "SDABalance") < 0 Then
                            lblSDABalance.ForeColor = Color.Red
                        End If

                        If DatabaseHelper.Peel_double(rd, "PFOBalance") < 0 Then
                            lblPFOBalance.ForeColor = Color.Red
                        End If
   ...
   End Sub
    Private Sub SetRollups()
        ...
        If Master.UserEdit Then
            ...
            If isSD Then
                Views.Add("<a style=""color:rgb(51,118,171);"" class=""lnk"" href=""" & ResolveUrl("~/clients/client/sdcalculator.aspx") & QueryString & """><img style=""margin-right:8px;"" border=""0"" runat=""server"" src=""" & ResolveUrl("~/images/16x16_calculator.png") & """ align=""absmiddle""/>SD Calculator</a>")
            End If
        ...
        If Not Tasks Is Nothing AndAlso Tasks.Count > 0 Then
            ...
            For Each t As Task In Tasks
                tdMessage.InnerHtml += "<li><font style=""color:black;"">DUE: " & t.Due.ToString("MM/dd/yyyy") _
                    & "</font> - " & t.Description & "&nbsp;&nbsp;<a class=""lnk"" href=""" _
                    & ResolveUrl("~/tasks/task/resolve.aspx?id=" & t.TaskID) & """ style=""color:#a1a1a1;font-size:9"">RESOLVE</a>"
            Next
    ...
    Protected Sub lnkResetPassword_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkResetPassword.Click
        Using cmd As New SqlCommand("exec stp_SetPassword " + ClientID.ToString() + " Temppass" + ClientID.ToString(), New SqlConnection(ConfigurationManager.AppSettings("CMSConnStr")))
            Using cmd.Connection
                cmd.Connection.Open()
                cmd.ExecuteNonQuery()
                cmd.Connection.Close()
                'MsgBox("Password set to Temppass" + ClientID.ToString(), vbSystemModal)
            End Using
        End Using
    End Sub
