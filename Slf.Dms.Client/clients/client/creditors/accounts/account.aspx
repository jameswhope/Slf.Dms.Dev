...
<asp:Content ID="cphBody" ContentPlaceHolderID="cphBody" runat="Server">
    ...
    function VerifyConfirm()
    {
        if (VerifyRequiredExist())
        {
            ...
            VerifiedAmount = parseFloat(txtVerifiedAmount.value);
             ...
             var url = '<%= ResolveUrl("~/clients/client/creditors/accounts/action/verify.aspx?id=" & ClientID & "&aid=" & AccountID) %>&t=Account Verification&a=' + VerifiedAmount;
	...
	{
         var url = '<%= ResolveUrl("~/clients/client/creditors/accounts/action/remove.aspx?id=" & ClientID & "&aid=" & AccountID) %>&t=Remove Account';
         window.dialogArguments = window;
         currentModalDialog = $('<div/>').appendTo("body").modaldialog({url: url,
	...
	{
         var url = '<%= ResolveUrl("~/clients/client/creditors/accounts/action/settle.aspx?id=" & ClientID & "&aid=" & AccountID) %>&t=Settle Account';
         window.dialogArguments = window;
         currentModalDialog = $('<div/>').appendTo("body").modaldialog({url: url,
	...
	function CI_DeleteConfirm(obj)
	{
        ...
        {
            window.dialogArguments = window;
            var url = '<%= ResolveUrl("~/util/pop/confirm.aspx")%>?f=CI_Delete&t=Delete Creditor Instances&m=Are you sure you want to delete this selection of creditor instances?';
            currentModalDialog = $('<div/>').appendTo("body").modaldialog({url: url,
                               title: "Delete Creditor Instances",
                               dialogArguments: window,
                               resizable: false,
                               scrollable: false,
                               height: 350, width: 400}); 
    ...
    function Record_AddMatter()
    {
        ...
        for (var c = 1; ; c++)
        {
            if (document.getElementById("chkCredCIID"+c)!=null)
            {
                var chk = document.getElementById("chkCredCIID"+c);
                
                if (chk.checked)
                {
                    val=document.getElementById("hdnCiid"+c).value;
                   count++;
                }
            }
            else 
            ...
            if(count==1 || count==0)
            {
                window.navigate("<%= ResolveUrl("~/clients/client/creditors/matters/addMatter.aspx?id=" & ClientID &"&aid=" & AccountID ) %>" + "&ciid="+val);
            }
           else if(count>1)
            {
                alert("Please select only one creditor instance");
                return false;
            }
    }
	function Record_DeleteConfirm()
	{
         var url = '<%= ResolveUrl("~/clients/client/creditors/accounts/action/delete.aspx?id=" & ClientID & "&aid=" & AccountID) %>&t=Delete Account';
         window.dialogArguments = window;
         currentModalDialog = $('<div/>').appendTo("body").modaldialog({url: url,
    ...
    {
         var url = '<%= ResolveUrl("~/clients/client/creditors/accounts/action/changestatus.aspx?id=" & ClientID & "&aid=" & AccountID) %>&t=Change Account Status';
         window.dialogArguments = window;
         currentModalDialog = $('<div/>').appendTo("body").modaldialog({url: url,
    ...
    function Record_Display(Message)
    {
        ...
        function AttachDocument()
        {
            ...
            attachWin = window.open('<%=ResolveUrl("~/util/pop/attachaccountdoc.aspx") %>?id=<%=ClientID %>&type=account&rel=<%=AccountID %>', 'AttachDocument', 'top=' + t + ',left=' + l + ',width=' + w + ',height=' + h + ',scrollbars=1');
                ...
                case "ltrrep":
                    if (document.getElementById("<%= lnkNonRepLetter.ClientID() %>").disabled !=true)
                    {
                        ...
                        if (strCreds.indexOf(",") > 0)
                        {
                            ...
                            {
                            strReports += 'LetterOfRepresentation_' + aCreds[i] + '_' + docid + ',';
                            }
                            strReports = strReports.substr(0,strReports.length-1)
                        }
                        else
                            {
                            strReports += 'LetterOfRepresentation_' + strCreds + '_' + docid ;
                            }
                ...
                case "nonrep":
                    if (document.getElementById("<%= lnkNonRepLetter.ClientID() %>").disabled !=true)
                    {
                        ...
                        if (strCreds.indexOf(",") > 0)
                        {
                            ...
                            for (i = 0; i < aCreds.length; i++)
                            {
                            strReports += 'NonRepresentationOfDebtLetter_' + aCreds[i] + '_' + docid + ',';
                            }
                            strReports = strReports.substr(0,strReports.length-1)
                        }
                        else
                            {
                            strReports += 'NonRepresentationOfDebtLetter_' + strCreds + '_' + docid;
                            }
                        ViewReport(strReports);
                    }                    
                ...
                case "date":
                    if (document.getElementById("<%= lnkDemandDateSpecific.ClientID() %>").disabled !=true)
                    {
                        ...
                        if (strCreds.indexOf(",") > 0)
                        {
                            ...
                            for (i = 0; i < aCreds.length; i++)
                            {
                            strReports += 'DemandDateSpecificLetter_' + aCreds[i] + '_' + txtDate.value + '_' + docid + ',';
                            }
                            strReports = strReports.substr(0,strReports.length-1)
                        }
                        else
                            {
                            strReports += 'DemandDateSpecificLetter_' + strCreds + '_' + txtDate.value + '_' + docid;
                            }
                        ViewReport(strReports);
                    }
                    break;
                case "nondate":
                    if (document.getElementById("<%= lnkDemandNonDateSpecific.ClientID() %>").disabled !=true)
                    {
                        ...
                        if (strCreds.indexOf(",") > 0)
                        {
                            ...
                            for (i = 0; i < aCreds.length; i++)
                            {
                            strReports += 'DemandNonDateSpecificLetter_' + aCreds[i] + '_' + docid + ',';
                            }
                            strReports = strReports.substr(0,strReports.length-1)
                        }
                        else
                            {
                            strReports += 'DemandNonDateSpecificLetter_' + strCreds + '_' + docid;
                            }
                        ViewReport(strReports);
                    }                    
                    break;
    ...
    function Grid_CheckOrUncheck(obj, id) {
        ...
        if (txtControls.length > 0) {
            var Controls = txtControls.nodeValue.split(",");

            for (c = 0; c < Controls.length; c++) {
                try {
                    document.getElementById(Controls[c]).disabled = txtSelected.length == 0;
                }
                catch (e) { }
            }
        }
    ...
    function OpenPABox(settid) {
        ...
        <table runat="server" id="tblBody" style="font-family:tahoma;font-size:11px;width:100%;" border="0" cellpadding="0" cellspacing="15">
                        ...
                        <tr>
                            <td valign="top">
                                <table style="width: 100%;" border="0" cellspacing="0" cellpadding="0">
                                    ...
                                    <td style="width:25;"><img id="Img2" runat="server" src="~/images/spacer.gif" width="25" height="1" border="0"/></td>
                                        ...
                                        <table runat="server" id="tblVerification" style="font-family:tahoma;font-size:11px;width:100%;height:100%;" border="0" cellpadding="0" cellspacing="0">
                                                <tr>
                                                    ...
                                                    <table style="background-color:#d3d3d3;width:100%;font-size:11px;font-family:Tahoma;" border="0" cellspacing="0" cellpadding="0">
                                                            <tr>
                                                            <td style="width:21;" valign="top"><img id="Img3" border="0" src="~/images/corner_gray_lt.gif" runat="server"/></td>
                                                            <td align="center"><img id="Img4" style="margin-right:3;" align="absmiddle" border="0" src="~/images/16x16_lock.png" runat="server"/>Balance Verification</td>
                                                            <td style="width:21;"><img id="Img5" border="0" src="~/images/corner_gray_rt.gif" runat="server"/></td>
                                                            </tr>
                                                        </table>
                                                    </td>
                                                </tr>
                                                    ...
                                                    <table style="font-size:11px;border-right:solid 1px #d3d3d3;border-left:solid 1px #d3d3d3;border-bottom:solid 1px #d3d3d3;" border="0" cellpadding="0" cellspacing="0" style="width:100%;">
                                                            ...
                                                            <tr>
                                                                ...
                                                                <asp:panel id="pnlVerificationAction" runat="server" style="background-color:#f1f1f1;padding:8 0 8 0;text-align:center;">
                                                                    <a id="A1" runat="server" href="javascript:VerifyConfirm();" class="lnk"><img id="Img6" border="0" align="absmiddle" runat="server" src="~/images/16x16_arrowright_clear.png" style="margin-right:5;"/>Click Here to Submit Verification</a></asp:panel>
                                        ...
                                        <td>
                                            <div id="dvTab0" runat="server" style="padding-top:10;">
                                                ...
                                                <table onmouseover="Grid_RowHover(this, true)" onmouseout="Grid_RowHover(this,false)" class="list" style="font-family:tahoma;font-size:11px;width:100%;" cellspacing="0" cellpadding="3">
                                                    ...
                                                    <thead>
                                                        <tr>
                                                            <th align="center" style="width:20;" class="headItem"><img id="Img7" onmouseup="this.style.display='none';this.nextSibling.style.display='inline';Grid_CheckAll(this);" style="cursor:pointer;" title="Check All" runat="server" src="~/images/11x11_checkall.png" border="0" /><img id="Img8" onmouseup="this.style.display='none';this.previousSibling.style.display='inline';Grid_UncheckAll(this);" style="cursor:pointer;display:none;" title="Uncheck All" runat="server" src="~/images/11x11_uncheckall.png" border="0" /></th>
                                                            <th style="width: 25;" align="center">
                                                                <img id="Img9" runat="server" src="~/images/16x16_icon.png" border="0" align="absmiddle" />
                                                            </th>
                                                            <th>Creditor</th>
                                                            <th>For Creditor</th>
                                                            <th>Acquired<img id="Img10" style="margin-left:5;" runat="server" src="~/images/sort-asc.png" border="0" align="absmiddle" /></th>
                                                            <th>Account #</th>
                                                            <th>Reference #</th>
                                                            <th>Original Amt.</th>
                                                            <th>Current Amt.</th>
                                                ...
                                                <input type="hidden" runat="server" id="txtSelectedIDs" />
                                                <input type="hidden" runat="server" id="txtControls" value="lnkDeleteConfirm" />
                                                <div>
                                                    <input type="hidden" value="<%= lnkDeleteConfirm.ClientId%>" />
                                                </div>
                                                <table class="entry">
                                                    <tr>
                                                                                                    ...
                                                                                                    <tr>
                                                                                                        ...
                                                                                                        <td align="left" style="width: 40%;">
                                                                                                            <a href="#" class="lnk" onclick="javascript:window.open('<%#CType(Container.DataItem.DocumentPath, String).Replace("\", "\\") %>');">
                                                                                                                <%#CType(Container.DataItem.DocumentType, String) %>&nbsp; </a>
                                                                                                        </td>
                                            ...
                                            <div id="dvTab1" runat="server" style="padding-top: 10; display: none">
                                                <!-- put negotiations content page here -->
                                            </div>
                                            <div id="dvTab2" runat="server" style="padding-top: 10; display: none">
                                            </div>
                                            <div id="dvTab3" runat="server" style="padding-top:10;display:none">
                                            <!-- put Matter content page here -->
                                        <table style="margin-top:15;background-color:#f3f3f3;font-family:tahoma;font-size:11px;width:100%;" cellpadding="0" cellspacing="0" border="0">
                                                    <tr>
                                                    <td style="padding:5 7 5 7;color:rgb(50,112,163);">Matters List</td>
                                                        <td style="padding-right: 7;" align="right">
                                                        <a class="lnk" id="lnkDeleteConfirm2" disabled="true" runat="server" href="#" onmouseup="javascript:MI_DeleteConfirm(this);">Delete</a>
                                                        </td>
                                                    </tr>
                                                </table>
                                                ...
                                                <table onmouseover="Grid_RowHover(this, true)" onmouseout="Grid_RowHover(this,false)" class="list" style="font-family:tahoma;font-size:11px;width:100%;" cellspacing="0" cellpadding="3">
                                                    ...
                                                    <thead>
                                                        <tr>
                                                            <th align="center" style="width:20;" class="headItem"><img id="imgMatterCheckAll" onmouseup="this.style.display='none';this.nextSibling.style.display='inline';Grid_CheckAll(this);" style="cursor:pointer;" title="Check All" runat="server" src="~/images/11x11_checkall.png" border="0" /><img id="imgMatterUnCheckAll" onmouseup="this.style.display='none';this.previousSibling.style.display='inline';Grid_UncheckAll(this);" style="cursor:pointer;display:none;" title="Uncheck All" runat="server" src="~/images/11x11_uncheckall.png" border="0" /></th>
                                                            <th style="width: 25;" align="center">
                                                                <img id="imgMatterHeader" runat="server" src="~/images/16x16_icon.png" border="0" align="absmiddle" />
                                                            </th>
                                                            <th>Create Date</th>
                                                            <th>Status<img id="imgStatus" style="margin-left:5;" runat="server" src="~/images/sort-asc.png" border="0" align="absmiddle" /></th>
                                                            <th>MatterNumber</th>
                                                            <th>MatterNote</th>
                                                            <th>Firm</th>
                                                            <th>Local Counsel</th>
                                                     ...
                                                     <asp:repeater runat="server" id="rpMatterInstance" OnItemDataBound="rpMatterInstance_ItemDataBound">
                                                            <itemtemplate>
                                                          <a href="<%#ResolveURL("~/clients/client/creditors/matters/matterinstance.aspx?id=" & ClientID & "&aid=" & AccountID &"&mid=" & DataBinder.Eval(Container.DataItem, "MatterID")&"&ciid="&DataBinder.Eval(Container.DataItem, "CreditorInstanceID"))  %>">
                                                                 <tr>
                                                                    <td style="width:20;" align="center"><img id="Img6" onmouseup="this.style.display='none';this.nextSibling.style.display='inline';this.nextSibling.nextSibling.checked=true;" runat="server" src="~/images/13x13_check_cold.png" border="0" align="absmiddle" /><img id="Img7" onmouseup="this.style.display='none';this.previousSibling.style.display='inline';this.nextSibling.checked=false;" style="display:none;" runat="server" src="~/images/13x13_check_hot.png" border="0" align="absmiddle" /><input id="chkCred" uysu;" style="display:none;" type="checkbox" /></td>
                                                                    <td ><img  alt= "" src="<%#ResolveURL("~/images/matter.jpg")%>" border="0"/></td>
                                                                    <td><%#DataBinder.Eval(Container.DataItem, "MatterDate", "{0:MM/dd/yyyy}")%>&nbsp;</td>
                                                                    <td><%#DataBinder.Eval(Container.DataItem, "MatterStatusCode")%>&nbsp;</td>
                                                                    <td><%#DataBinder.Eval(Container.DataItem, "MatterNumber")%>&nbsp;</td> 
                                                                    <td><%#DataBinder.Eval(Container.DataItem, "MatterMemo")%>&nbsp;</td> 
                                                                    <td><%#DataBinder.Eval(Container.DataItem, "Firm")%>&nbsp;</td>
                                                                    <td><%#DataBinder.Eval(Container.DataItem, "Attorney")%>&nbsp;</td>  
                                                                    <td><font color="blue" ><a class="lnk" href="<%#ResolveURL("~/clients/client/creditors/matters/matterroadmap.aspx?id=" & ClientID & "&aid=" & AccountID &"&mid=" & DataBinder.Eval(Container.DataItem, "MatterID")) %>">TimeLine</a></font>&nbsp;</td>
                                                            ...
                                                            <FooterTemplate>
                                                              <tr>
                                                              <td></td>
                                                              <td> <asp:Label ID="lblEmptyData" Text="No Data to Display" runat="server" Visible="false"></asp:Label></td>
                                                              <td></td>
                                                              <td></td>
                                                              <td></td>
                                                              <td></td>
                                                         ...
                                                         <!--
                                                            <tr>
														    <td colspan="9">
															    <a class="lnk" onclick="javascript:Print_PreJudgment();" id="A2"
																    disabled="false" runat="server" href="#">Pre Judgment Intake</a>&nbsp;|&nbsp;
															    <a class="lnk" onclick="javascript:Print_PostJudgment();" id="A3"
																    disabled="false" runat="server" href="#">Post Judgment Intake</a>&nbsp;|&nbsp;
													        </td>
                                                                ...
                                                                <table id="Table1" style="font-family: tahoma; font-size: 11px; width: 100%;"
                                                                    border="0" cellpadding="5" cellspacing="0" runat="server">
                                                                        <tr id="tr3" runat="server">
                                                                            ...
                                                                            <td style="background-color: #f1f1f1;" align="right">
                                                                                <a class="lnk" href="javascript:AttachDocument();">Attach Document</a>&nbsp;|&nbsp;<a
                                                                                class="lnk" id="lnkMatterDeleteDocuments" disabled="true" runat="server" href="#" onmouseup="javascript:DeleteDocument();">Delete</a>
                                                                            </td>
                                                                        </tr>
                                                                        <tr id="tr4" runat="server">
                                                                            <td colspan="2">
                                                                                <table onmouseover="RowHover(this, true)" onmouseout="RowHover(this,false)" class="list"
                                                                                    ...
                                                                                    <tbody>
                                                                                        <asp:Repeater runat="server" ID="rpMatterDocuments">
                                                                                            <ItemTemplate>
                                                                                                <a href="#" onclick="javascript:RowClick(this.childNodes(0), <%#CType(Container.DataItem.DocRelationID, Integer) %>);">
                                                                                                    <tr>
                                                                                                        <td style="width: 20px;" align="center">
            ...
            <tr>
                ...
                <td>
                    <img id="Img12" src="~/images/loading.gif" runat="server" align="absmiddle" border="0" />
                </td>
            </tr>
        </table>
