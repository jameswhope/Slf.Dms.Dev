...
Partial Class clients_client_accounts_account
    ...
    Public Function GetAttachmentsForRelation(ByVal ClientID As Integer, ByVal AccountID As Integer, ByVal relationType As String, Optional ByVal url As String = "") As List(Of AttachedDocument)
        ...
        Dim cmdStr As String = " SELECT dr.RelationID, dr.DocRelationID, dt.DisplayName, isnull(ds.ReceivedDate, '01/01/1900') as ReceivedDate, " & _
         ...
         " WHERE dr.RelationID  in (select m.MatterId from   " & _
        " dbo.tblMatter m join dbo.tblClient c on c.ClientId=m.ClientId  " & _
        " join tblAccount ac on ac.ClientId=c.ClientId and ac.AccountId= '" & AccountID & "'  " & _
        " join tblCreditorInstance ti on ti.CreditorInstanceId=m.CreditorInstanceId   " & _
        " and ti.AccountId= ac.AccountId  where c.ClientId='" & ClientID & "' and IsNull(m.IsDeleted,0)=0  " & _
        " )  and dr.RelationType = 'matter' and (DeletedFlag = 0 or DeletedBy = -1)  " & _
