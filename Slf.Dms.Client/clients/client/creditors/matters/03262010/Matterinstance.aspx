...
//end
    ...
    {
      var r= showModalDialog("<%= ResolveUrl("~/util/pop/holder.aspx") %>?t=Add Matter Task &a=m&p=<%= ResolveUrl("~/tasks/task/propagations.aspx") %>&mid=<%=MatterId%>&cid=<%=ClientID%>&type=<%=MatterTypeId%>", window, "status:off;help:off;dialogWidth:850px;dialogHeight:450px");
      if(r==1)
        {
    ...
    {
       var r= showModalDialog("<%= ResolveUrl("~/util/pop/holder.aspx") %>?t=Edit Matter Task &a=m&p=<%= ResolveUrl("~/tasks/task/propagations.aspx") %>&mid=<%=MatterId%>&cid=<%=ClientID%>&type=<%=MatterTypeId%>", window, "status:off;help:off;dialogWidth:850px;dialogHeight:450px");
        if(r==1)
        {
            window.location=window.location;
        }
    ...
    function Record_Display(Message)
    {
        ...
        */
        function AttachDocument()
        {
            ...
            attachWin =  showModalDialog("<%= ResolveUrl("~/util/pop/holder.aspx") %>?t=Attach Document&a=m&p=<%= ResolveUrl("~/util/pop/attachmatterdocument.aspx") %>&id=<%= ClientID %>&typeid=<%=MatterTypeId%>&type=matter&aid=<%=request.querystring("aid")%>&rel="+relID, window, "status:off;help:off;dialogWidth:850px;dialogHeight:450px");
            if(attachWin ==-1)
            {
        ...
        {
            //write code here
          attachWin =  showModalDialog("<%= ResolveUrl("~/util/pop/holder.aspx") %>?t=Client Intake Form&a=m&p=<%= ResolveUrl("~/clients/client/creditors/matters/Intake.aspx") %>&id=<%=request.querystring("id")%>&typeid=<%=MatterTypeId%>&ciid=<%=request.querystring("ciid")%>&mid=<%=request.querystring("mid")%>&aid=<%=request.querystring("aid")%>", window, "status:off;help:off;dialogWidth:950px;dialogHeight:740px");
          if(attachWin ==-1)
          {
        ...
        function UploadDocument()
        {
            ...
            //scanWin = window.open('<%=ResolveUrl("~/util/pop/Uploadmatterdocument.aspx") %>?id=<%=ClientID %>&type=matter&temp=1' + '&rel=' + relID, 'UploadDocument', 'top=' + t + ',left=' + l + ',width=' + w + ',height=' + h + ',scrollbars=1');
            
            attachWin =  showModalDialog("<%= ResolveUrl("~/util/pop/holder.aspx") %>?t=Upload Matter Document&a=m&p=<%= ResolveUrl("~/util/pop/Uploadmatterdocument.aspx") %>&type=matter&temp=1&id=<%=request.querystring("id")%>&rel=<%=hdnTempMatterID.value %>&ciid=<%=CreditorInstanceId %>&aid=<%=request.querystring("aid")%>", window, "status:off;help:off;dialogWidth:650px;dialogHeight:300px");
            if(attachWin ==-1)
            {
                            ...
                            <td style="background-color: #f1f1f1;" align="right">
                               <span id="dvActions" runat="Server" style="display:none"> <a style="color: rgb(51,118,171);" href="javascript:UploadDocument();" class="lnk">Upload
                                    Doc</a>&nbsp;|&nbsp;<a class="lnk" href="javascript:CreateIntake();">Client Intake Form</a>&nbsp;|&nbsp;</span><a
                                        class="lnk" href="javascript:AttachDocument();">Attach Document</a></a>&nbsp;|&nbsp;<a
