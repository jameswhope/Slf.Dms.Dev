...
Partial Class clients_client_creditors_matters_NonDepositMatterInstance
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not qs Is Nothing Then
            ...
            If Not IsPostBack Then
                ...
                LoadMatterDetail()
                'Add resolve link for non deposit type
            ...
            Using cmd.Connection
                ...
                Using rd As IDataReader = cmd.ExecuteReader(CommandBehavior.SingleResult)
                    While rd.Read()
                        ...
                        txtMatterDate.Enabled = False
                        txtMatterNumber.Text = rd("MatterNumber")
                        txtMatterNumber.Enabled = False
                        ...
                        ddlMatterStatusCode.Enabled = False
                        ...
                        ddlMatterSubStatusCode.Enabled = False
    ...
    Private Sub SetRollups()
        ...
        CommonTasks.Add("<a style=""color:rgb(51,118,171);"" class=""lnk lnksave"" href=""#"" onclick=""Record_SaveConfirm();return false;""><img style=""margin-right:8px;"" border=""0"" runat=""server"" src=""" & ResolveUrl("~/images/16x16_save.png") & """ align=""absmiddle""/>Save Matter</a>")
        CommonTasks.Add("<a style=""color:rgb(51,118,171);"" class=""lnk lnksave"" href=""#"" onclick=""Record_SaveAndClose();return false;""><img style=""margin-right:8px;"" border=""0"" runat=""server"" src=""" & ResolveUrl("~/images/16x16_save.png") & """ align=""absmiddle""/>Save and close</a>")
        ...
        If Master.UserEdit Or UserTypeID = 5 Then 'Attorneys always can add notes
            ...
            If UserGroupID = 20 Or UserGroupID = 34 Or UserGroupID = 35 Or UserGroupID = 38 Or UserGroupID = 41 Then
        ...
        If NonDepositHelper.CanResolveNonDepositMatter(MatterId) Then
            CommonTasks.Add("<a style=""color:rgb(51,118,171);"" class=""lnk"" href=""#"" onclick=""javascript:ShowResNonDeposit('" & MatterId & "');return false;""><img style=""margin-right:8px;"" border=""0"" runat=""server"" src=""" & ResolveUrl("~/images/16x16_taskresolution.png") & """ align=""absmiddle""/>Resolve Matter</a>")
        End If
        'Non Deposit-Change Resolve
        If NonDepositHelper.CanReResolveNonDepositMatter(MatterId) AndAlso PermissionHelperLite.HasPermission(UserID, "Non Deposit-Change Resolve") Then
            CommonTasks.Add("<a style=""color:rgb(51,118,171);"" class=""lnk"" href=""#"" onclick=""javascript:ShowResNonDeposit('" & MatterId & "');return false;""><img style=""margin-right:8px;"" border=""0"" runat=""server"" src=""" & ResolveUrl("~/images/16x16_taskresolution.png") & """ align=""absmiddle""/>Change Resolve</a>")
        End If
    ...
    Private Sub InsertOrUpdate()
        ...
        Dim MatterStatusCodeId As Integer = DataHelper.FieldLookup("tblMatter", "MatterStatusCodeId", "MatterID = " & MatterId)
        ...
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            Using cmd.Connection
                ...
                DatabaseHelper.AddParameter(cmd, "AccountId", 0)
                ...
                DatabaseHelper.AddParameter(cmd, "AttorneyId", -1)
                DatabaseHelper.AddParameter(cmd, "CreditorInstanceId", -1)
                DatabaseHelper.AddParameter(cmd, "MatterTypeId", ddlMatterType.SelectedValue)
                DatabaseHelper.AddParameter(cmd, "MatterStatusId", ddlMatterStatusCode.SelectedValue)
                DatabaseHelper.AddParameter(cmd, "MatterSubStatusId", ddlMatterSubStatusCode.SelectedValue)
                DatabaseHelper.AddParameter(cmd, "MatterStatusCodeId", MatterStatusCodeId)
    ...
    Private Sub LoadNonDepositData()
        Dim dt As DataTable = NonDepositHelper.GetNonDepositData(MatterId)
        If Not dt Is Nothing AndAlso dt.Rows.Count > 0 Then
            Dim dr As DataRow = dt.Rows(0)
            lblNonDepositType.Text = dr("NonDepositType").ToString.ToUpper
            lblDepositDate.Text = CDate(dr("DepositDate")).ToShortDateString
            lblDepositAmount.Text = String.Format("{0:c}", dr("DepositAmount"))
            If dr("NonDepositType").ToString.Trim.ToLower = "bounced" Then
                lblBouncedDate.Text = CDate(dr("BouncedDate")).ToShortDateString
                lblbounceReason.Text = dr("BouncedReason")
                ltrDepositNumber.Text = String.Format("<a href='{0}?id={1}&rid={2}'>{2}</a>", ResolveUrl("~/clients/client/finances/bytype/register.aspx"), ClientID, dr("depositId"))
                Me.trBounceDate.Visible = True
                Me.trBouncedReason.Visible = True
                Me.trDepositId.Visible = True
            Else
                Me.trBounceDate.Visible = False
                Me.trBouncedReason.Visible = False
                Me.trDepositId.Visible = False
            End If
        End If
    End Sub
