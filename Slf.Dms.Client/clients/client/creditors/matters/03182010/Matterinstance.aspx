    ...
    function Record_Display(Message)
    {
        ...
        */
        function AttachDocument()
        {
            ...
            attachWin =  showModalDialog("<%= ResolveUrl("~/util/pop/holder.aspx") %>?t=Attach Document&a=m&p=<%= ResolveUrl("~/util/pop/attachmatterdocument.aspx") %>&id=<%= ClientID %>&typeid=<%=MatterTypeId%>&type=matter&aid=<%=request.querystring("aid")%>&ciid=<%=request.querystring("ciid")%>&rel="+relID, window, "status:off;help:off;dialogWidth:850px;dialogHeight:450px");
            if(attachWin ==-1)
            {
