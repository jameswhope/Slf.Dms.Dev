...
#End Region
    ...
    Private Sub LoadTabStrips()
        ...
        If hdnCBEmailCount.Value <> "" And hdnCBEmailCount.Value <> "0" Then
            tsMatterView.TabPages.Add(New Slf.Dms.Controls.TabPage("Checks&nbsp;By&nbsp;Email&nbsp;&nbsp;<font color='blue'>(" & hdnCBEmailCount.Value.ToString() & ")</font>", dvPanel5.ClientID))
        Else
            tsMatterView.TabPages.Add(New Slf.Dms.Controls.TabPage("Checks&nbsp;By&nbsp;Email", dvPanel5.ClientID))
        End If
    ...
    Private Sub PopulateMatterTypes(ByVal MatterTypeId As Integer)
        ...
        Select Case MatterTypeId
            Case 1
                trCaseNum.Style("display") = "block"
            Case Else
                trCaseNum.Style("display") = "none"
        End Select
            ...
            Using cmd.Connection
                ...
                Using rd As IDataReader = cmd.ExecuteReader()
                    While rd.Read
                        ...
                        tsk.TaskTypeId = DatabaseHelper.Peel_int(rd, "TaskTypeId")
                        Tasks.Add(tsk)
    ...
    Private Sub LoadOverview()
        ...
        DatabaseHelper.AddParameter(cmd, "ReturnTop", "100")
        DatabaseHelper.AddParameter(cmd, "MatterID", MatterId)
    ...
    Private Sub LoadChecksByEmail()

        Dim dt As DataTable = SettlementMatterHelper.GetChecksByEmailHistory(MatterId)
        Me.rptChecksByEMail.DataSource = dt
        Me.rptChecksByEMail.DataBind()

        hdnCBEmailCount.Value = dt.Rows.Count
        pnlNoCBE.Visible = (dt.Rows.Count = 0)
            ...
            Using cmd.Connection
                ...
                Using rd As IDataReader = cmd.ExecuteReader(CommandBehavior.SingleResult)
                    While rd.Read()
                        ...
                        txtMatterCaseNumber.Text = rd("mattercasenumber").ToString
                        ...
                        hdnPrevMatterStatus.Value = ddlMatterStatusCode.SelectedValue
                        ...
                        hdnPrevSubStatus.Value = ddlMatterSubStatusCode.SelectedValue
                    End While
                End Using
            End Using
        End Using

        If MatterTypeId = 3 Then
            Dim stip As String = DataHelper.FieldLookup("tblSettlements", "IsClientStipulation", "MatterId = " & MatterId).Trim.ToLower
            Me.hdnStipulation.Value = IIf(stip = "true", "1", "0")
            Dim settlementId As Integer = CInt(DataHelper.FieldLookup("tblSettlements", "SettlementId", "MatterId = " & MatterId))
            Dim IsPA As Boolean = CBool(DataHelper.FieldLookup("tblSettlements", "IsPaymentArrangement", "SettlementId = " & settlementId))
            If IsPA Then
                Me.lblPayments.Text = String.Format("&nbsp;&nbsp;<a class=""lnkPA"" href=""javascript:void()"" onclick=""OpenPABox({0});return false;"">Payment&nbsp;Terms</a>", settlementId)
            End If
        End If
    End Sub
    ...
    Private Sub PopulateClassifications()
        ...
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            Using cmd.Connection
                cmd.CommandText = "SELECT * FROM tblClassifications with(nolock) Where Display = 1"
    ...
    Private Sub PopulateMatterClassifications()
        ...
        Using dt As DataTable = SqlHelper.GetDataTable(String.Format("SELECT ClassificationID FROM tblMatterClassification with(nolock) Where MatterID={0} and deleted is null", MatterId), CommandType.Text)
            For Each row As DataRow In dt.Rows
                Integer.TryParse(row("ClassificationID").ToString(), ClassificationID)
                Dim li As ListItem = ddlClassification.Items.FindByValue(ClassificationID)
        ...
        If iCnt = 0 Then
            ...
            tdError.InnerHtml = "There are no classifications assigned for this matter!"
        End If
    ...
    Private Sub SetRollups()
        ...
        If Master.UserEdit Or UserTypeID = 5 Then 'Attorneys always can add notes
            ...
            Select Case UserGroupID
                Case 11, 20, 30, 34, 35, 36, 37, 38, 41, 42
    ...
    Private Sub InsertOrUpdate()
        ...
        Dim MatterStatusCodeId As Integer = 0
        ...
        If MatterTypeId = 3 Then
            MatterStatusCodeId = CInt(DataHelper.FieldLookup("tblMatterStatusCode", "MatterStatusCodeId", "MatterStatusCode = '" & ddlMatterSubStatusCode.SelectedItem.Text.Replace("'", "\'") & "'"))
        End If

        If ddlMatterSubStatusCode.SelectedValue = 70 And hdnPrevMatterStatus.Value = "4" Then
            Throw New Exception("Error. I.T. has to complete this task") 'Need to convert this to accomodate P.A.
            Dim RegisterId() As Integer
            Dim PaymentId As Integer = CInt(DataHelper.FieldLookup("tblAccount_PaymentProcessing", "PaymentProcessingId", "MatterId = " & MatterId))
            Dim CheckNumber As Integer = CInt(DataHelper.FieldLookup("tblAccount_PaymentProcessing", "CheckNumber", "PaymentProcessingId = " & PaymentId))
            Dim CheckAmount As Integer = CDbl(DataHelper.FieldLookup("tblAccount_PaymentProcessing", "CheckAmount", "PaymentProcessingId = " & PaymentId))
            Dim SettRegisterId As Integer = CInt(DataHelper.FieldLookup("tblFirmRegister", "RegisterId", "CheckNumber = " & CheckNumber))
            RegisterId = DataHelper.FieldLookupIDs("tblRegister", "RegisterId", "RegisterSetId = " & SettRegisterId)
            SettlementMatterHelper.VoidSettlementAmount(PaymentId, SettRegisterId, CheckNumber, CheckAmount, UserID)

            'Mark FirmRegister entry as Voided
            Using cmd As New SqlCommand("UPDATE tblFirmRegister SET Void = 1, VoidedDate=getdate(), LastModified = getdate()," & _
                                        " LastModifiedBy =" & UserID & " WHERE RegisterId = " & SettRegisterId, ConnectionFactory.Create())
                Using cmd.Connection
                    cmd.Connection.Open()
                    cmd.ExecuteNonQuery()
                End Using
            ...
            RegisterHelper.Void(SettRegisterId, UserID, True)
            If Not RegisterId Is Nothing AndAlso RegisterId.Length > 0 Then
                RegisterHelper.Void(RegisterId, UserID, True)
            End If
        ...
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            Using cmd.Connection
                ...
                DatabaseHelper.AddParameter(cmd, "MatterStatusCodeId", IIf(MatterStatusCodeId = 0, ddlMatterStatusCode.SelectedValue, MatterStatusCodeId))
                ...
                'update matter case number
                If MatterTypeId = 1 Then
                    Dim sqlUpdate As String = String.Format("update tblmatter set mattercasenumber = '{0}' where matterid = {1}", txtMatterCaseNumber.Text, MatterId)
        ...
        'delete old classifications if any and 
        Dim sqlDelete As String = String.Format("update tblMatterClassification set deleted=getdate() WHERE MatterID = {0}", MatterId)
        SqlHelper.ExecuteNonQuery(sqlDelete.ToString, CommandType.Text)
        ...
        For iListIndex = 0 To ddlClassification.Items.Count - 1
            If ddlClassification.Items(iListIndex).Selected Then
                ...
                sqlInsert.AppendFormat("INSERT INTO tblMatterClassification SELECT {0},{1},{2} ,getdate(),null  ", MatterId, ddlClassification.Items(iListIndex).Value, UserID)
                SqlHelper.ExecuteNonQuery(sqlInsert.ToString, CommandType.Text)
            End If
        Next iListIndex
    ...
    Private Sub LoadDocuments()
        ...
        rpDocuments.DataSource = GetAttachmentsForRelation(Integer.Parse(hdnTempMatterID.Value), "matter", Request.Url.AbsoluteUri.ToLower())
        rpDocuments.DataBind()
    ...
    Private Sub LoadLegacyDocs()
        ...
        'LegacyDocumentRoot = "\\TS-RXL2AC\files\" + AccountNumber.ToString() + "\1191\"
        'LegacyDocumentRoot = "\\TS-RXL2AC\files\" + AccountNumber.ToString() + "\" + Right(CreditorAcctnum.ToString(), 4) + "\"

        If Directory.Exists(LegacyDocumentRoot) Then

            'LegacyDocumentRoot = "\\TS-RXL2AC\files" + "\" + AccountNumber
    ...
    Public Function BuildAttachmentPath(ByVal docTypeID As String, ByVal docID As String, ByVal dateStr As String, ByVal clientID As Integer, Optional ByVal subFolder As String = "") As String
        ...
        If docTypeID = "M030" Then
            ...
            Return subFolder
        ElseIf docTypeID = "9074" Then
            If File.Exists("\\" + server + "\" + storage + "\" + acctNo + "\" + folder + "\" + IIf(subFolder.Length > 0, subFolder, "") + acctNo + "_" + docTypeID + "_" + docID + "_" + dateStr + ".mp3") Then
                Return "\\" + server + "\" + storage + "\" + acctNo + "\" + folder + "\" + IIf(subFolder.Length > 0, subFolder, "") + acctNo + "_" + docTypeID + "_" + docID + "_" + dateStr + ".mp3"
            Else
                Return "\\" + server + "\" + storage + "\" + acctNo + "\" + folder + "\" + IIf(subFolder.Length > 0, subFolder, "") + acctNo + "_" + docTypeID + "_" + docID + "_" + dateStr + ".wav"
            End If
        Else
    ...
    ''' <summary>
    ''' changes root of documents to share when accessed from outside company
    ''' </summary>
    ''' <param name="dirPath"></param>
    ''' <returns>corrected path</returns>
    ''' <remarks></remarks>
    Public Function BuildDirectory(ByVal dirPath As String) As String
        Dim dirString As String = "dir"
        Try
            If Request.Url.AbsoluteUri.ToLower().Contains(".com") Then
                dirString = dirPath.Replace("\\\\TS-RXL2AC\\files", "http://services.dmsisupport.com/TS-RXL2AC").Replace("\\", "\").Replace("\", "/")
            Else
                dirString = dirPath
            End If
