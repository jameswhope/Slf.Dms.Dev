...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not qs Is Nothing Then
            ...
            If Not IsPostBack Then
                ...
                End If
                'monthlyFee.SelectedValue = Integer.Parse(DataHelper.FieldLookup("tblClient", "MonthlyFee", "ClientID = " & ClientID)).ToString()
         ...
         Using cmd.Connection
            ...
            Using rd As IDataReader = cmd.ExecuteReader
               ...
               If rd.Read Then
                        ...
                        cboBankType.SelectedValue = DatabaseHelper.Peel_string(rd, "BankType").Trim
                        ...
                        If DatabaseHelper.Peel_decimal(rd, "MaintenanceFeeCap") > 0 Then
                            monthlyFee.Visible = False
                            Me.plhMonthlyFee.Visible = True
                            Dim accts As Integer = GetActiveAccts()
                            Dim calcFee As Double = accts * DatabaseHelper.Peel_double(rd, "MonthlyFee")
                            Dim cap As Double = DatabaseHelper.Peel_double(rd, "MaintenanceFeeCap")
                            Me.txtMonthlyFee.Text = String.Format("{0:###0.00}", IIf(calcFee > cap, cap, calcFee))
                            Me.lblMonthlyFee.Text = String.Format(" = {0} acc. x {1:c}, max {2:c}", accts, DatabaseHelper.Peel_double(rd, "MonthlyFee"), cap)
                        Else
                            monthlyFee.Visible = True
                            Me.plhMonthlyFee.Visible = False
   ...
   End Sub
    Private Sub LoadAccountsGrid(ByVal Accounts As List(Of Slf.Dms.Records.Account))
        ...
        sb.Append("         <th fieldName=""Street"" fieldType=""varchar"" mode=""txt"" control=""0""><div class=""header""><div>Billing Street</div></div></th>")
        sb.Append("         <th fieldName=""Street2"" fieldType=""varchar"" mode=""txt"" control=""0""><div class=""header""><div>Street 2</div></div></th>")
        sb.Append("         <th fieldName=""City"" fieldType=""varchar"" mode=""txt"" control=""0""><div class=""header""><div>City</div></div></th>")
        sb.Append("         <th fieldName=""StateID"" fieldType=""int"" mode=""ddl"" control=""1""><div class=""header""><div>State</div></div></th>")
        sb.Append("         <th fieldName=""ZipCode"" fieldType=""varchar"" mode=""txt"" control=""0""><div class=""header""><div>Zip Code</div></div></th>")
   ...
   Private Sub Save(ByVal Resolving As Boolean)
      ...
      'If cboDepositMethod.SelectedItem.Value.ToUpper = "ACH" Then
      '   If Not objStore.RoutingIsValid(txtRoutingNumber.Text, lblBankName.Text) Then
      '      txtRoutingNumber.Style("border") = "solid 2px red"
      '      dvErrorSetup.Style("display") = ""
      '      tdErrorSetup.InnerHtml = "The Bank Routing Number you entered does not validate against the Federal ACH Directory."
      '      Exit Sub
      '   End If
      'End If

        StateID = Me.cboStateID.SelectedValue
        If StateID = "1" Or StateID = "34" Or StateID = "42" Then
      ...
      If Resolving Then
         Dim TrustId As Integer = DataHelper.FieldLookup("tblClient", "TrustId", "ClientID = " & ClientID)
         Dim acctNo As String = DataHelper.FieldLookup("tblClient", "AccountNumber", "ClientID = " & ClientID)

         If (acctNo.Length = 0 Or acctNo = "0") And TrustId = 22 Then
            Dim AccountNumber As String = GetNextAccountNumber()
      ...
      End If
        ...
        'Clean Up after updating accounts
   ...
   Private Sub SetupUnderwriting()
      ...
      End If
        ...
        TaskHelper.Resolve("tblClientTask.ClientID = " & ClientID & " AND tblTask.TaskTypeID = 12", 1, UserID) '12=Resolve Data Entry
   End Sub
   ...
   Private Sub UpdateClient(ByVal Resolving As Boolean)
        ...
        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            ...
            If Val(DataHelper.FieldLookup("tblClient", "MaintenanceFeeCap", "ClientId = " & ClientID)) = 0 Then
                DatabaseHelper.AddParameter(cmd, "MonthlyFee", DataHelper.Zn_double(monthlyFee.SelectedValue), DbType.Decimal)
            End If

            If ddlRetainer.SelectedValue = "10" Then
                DatabaseHelper.AddParameter(cmd, "InitialAgencyPercent", "0.02", DbType.Decimal)
            Else
                DatabaseHelper.AddParameter(cmd, "InitialAgencyPercent", Nothing, DbType.Decimal)
            End If

            If cboBankType.SelectedValue = "0" Then
                DatabaseHelper.AddParameter(cmd, "BankType", DBNull.Value, DbType.String)
            Else
                DatabaseHelper.AddParameter(cmd, "BankType", cboBankType.SelectedValue.Trim, DbType.String)
            End If
   ...
   Private Function GetNextAccountNumber() As String
   ...
   Protected Sub lnkReturn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkReturn.Click
    ...
    Private Function GetActiveAccts() As Double
        Dim fee As Double = 0

        Using cmd As IDbCommand = ConnectionFactory.Create().CreateCommand()
            cmd.CommandText = "select accts = count(a.accountid) from tblaccount a	where a.clientid = @clientid and a.accountstatusid <> 55 and not (a.accountstatusid = 54 and exists(select r.registerid from tblregister r where r.entrytypeid = 4 and r.accountid = a.accountid and r.void is null and r.isfullypaid = 1))"
            DatabaseHelper.AddParameter(cmd, "ClientID", ClientID)
            Using cmd.Connection
                cmd.Connection.Open()
