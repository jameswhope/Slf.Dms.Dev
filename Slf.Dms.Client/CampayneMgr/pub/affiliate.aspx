<%@ Page Title="" Language="VB" MasterPageFile="~/campayne.master" AutoEventWireup="false"
    CodeFile="affiliate.aspx.vb" Inherits="admin_affiliate" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
                    ...
                    preinit : {
                        UploadFile: function(up, file) {
                            // You can override settings before the file is uploaded
                             up.settings.url = '../handlers/FileUploaderHandler.ashx?type=affiliate';
                             up.settings.multipart_params = {affiliateid : $("#dialog-affiliate").data('affiliateid')}
                        }
                    },
                    init: {  
    ...
    <script type="text/javascript">
        function getCampaignData(tab) {
            $('#divCampaignGrid').html(loadingImg);
            var aid = $("#dialog-affiliate").data('affiliateid');
            var dArray = "{'affiliateid': '" + aid + "'}";
            $.ajax({
                type: "POST",
                url: "../service/cmService.asmx/getCampaigns",
                data: dArray,
        ...
        function ShowAffiliate(affiliateid, name, accountmanager, website, billingcycle, street, city, state, zip, country, notes, active, lastmodified) {
            $("*[id$='txtName']").val(name);
            ...
            $("*[id$='lblLastModified']").text(lastmodified);
            if (active.toLowerCase() == 'true') {
                $("*[id$='chkActive']").attr('checked', true);
            } else {
                $("*[id$='chkActive']").attr('checked', false);
            }
            $("#affiliatetab").tabs({ selected: 0 });
            $("#dialog-affiliate")
                .data('affiliateid', affiliateid)
                .dialog("open");

            if (affiliateid > 0) {
                name = 'Affiliate #' + affiliateid + ' - ' + name;
            }
            $("#dialog-affiliate").dialog("option", "title", name);
        }

        function SaveAffiliate() {
            var newaffiliate = new Object();
            newaffiliate.AffiliateID = $("#dialog-affiliate").data('affiliateid');
            newaffiliate.Name = $("*[id$='txtName']").val();
            newaffiliate.AccountManager = $("*[id$='txtAcctMgr']").val();
            newaffiliate.Website = $("*[id$='txtWebsite']").val();
            newaffiliate.BillingCycle = $("*[id$='ddlBillingCycle']").val();
            newaffiliate.Street = $("*[id$='txtStreet']").val();
            newaffiliate.City = $("*[id$='txtCity']").val();
            newaffiliate.State = $("*[id$='ddlState']").val();
            newaffiliate.Zip = $("*[id$='txtZip']").val();
            newaffiliate.Country = $("*[id$='ddlCountry']").val();
            newaffiliate.Notes = $("*[id$='txtNotes']").val();
            newaffiliate.Active = getCheckboxValue($("*[id$='chkActive']").attr("checked"));

            var DTO = { 'newaffiliate': newaffiliate };

            $.ajax({
                type: "POST",
                url: "../service/cmService.asmx/InsertUpdateAffiliate",
    ...
    <script type="text/javascript">
        ...
        function SaveContact() {
            ...
            newcontact.ParentID = $("#dialog-affiliate").data('affiliateid');
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div style="float: right; padding: 0px 0px 5px 5px">
        <table cellpadding="0" cellspacing="0">
            <tr>
                <td>
                    <asp:UpdateProgress ID="UpdateProgress1" runat="server" AssociatedUpdatePanelID="upOffer">
                        <ProgressTemplate>
                ...
                <td>
                    <small>
                        <input type="submit" class="jqButton" onclick="return ShowAffiliate(-1, 'New Affiliate', '-1', '', '', '', '', '', '', 'USA', '', 'false');" value="Create Affiliate" />
                    </small>
                </td>
            </tr>
        </table>
    ...
    <div class="portlet">
        <div class="portlet-content">
                    <asp:GridView ID="gvAffiliates" runat="server" DataSourceID="odsAffiliates" AutoGenerateColumns="False"
                        Width="100%" AllowPaging="True" AllowSorting="True" PageSize="22" GridLines="None" AlternatingRowStyle-CssClass="altrow">
                        <RowStyle VerticalAlign="Top" />
                        <Columns>
                            <asp:BoundField DataField="AffiliateID" HeaderText="ID" SortExpression="AffiliateID">
                                <HeaderStyle HorizontalAlign="left" CssClass="ui-widget-header" />
                                <ItemStyle HorizontalAlign="left" CssClass="griditem2" Width="30px" />
                            </asp:BoundField>
                            <asp:TemplateField HeaderText="Affiliate" SortExpression="Name">
                                <ItemTemplate>
                                ...
                                <ItemStyle HorizontalAlign="Left" CssClass="griditem2" Width="320px" />
                            </asp:TemplateField>
                            <asp:BoundField DataField="AccountManager" HeaderText="Account Mgr" SortExpression="AccountManager">
                                <HeaderStyle HorizontalAlign="left" CssClass="ui-widget-header" />
                                <ItemStyle HorizontalAlign="left" CssClass="griditem2" Width="180px" />
                            </asp:BoundField>
                            <asp:BoundField DataField="BillingCycle" HeaderText="Billing&nbsp;Cycle" SortExpression="BillingCycle">
                                <HeaderStyle HorizontalAlign="left" CssClass="ui-widget-header" />
                                <ItemStyle HorizontalAlign="left" CssClass="griditem2" Width="150px" />
                            </asp:BoundField>
                    ...
                    <asp:ObjectDataSource ID="odsAffiliates" runat="server" OldValuesParameterFormatString="original_{0}"
                        SelectMethod="getAffiliates" TypeName="AdminHelper+AffiliateObject">
                        <SelectParameters>
                            <asp:Parameter Name="sortField" Type="String" />
                            <asp:Parameter Name="sortOrder" Type="String" />
                            <asp:Parameter Name="affiliateID" Type="Int32" DefaultValue="-1" />
                        </SelectParameters>
    ...
    <div id="dialog-affiliate" title="Affiliate">
        <div id="affiliatetab">
            <ul>
                <li><a href="#btab0"><span>General Info</span></a></li>
                <li><a href="#btab1"><span>Campaigns</span></a></li>
            ...
            <div id="btab0">
                <table style="width: 100%">
                    <tr>
                        <td class="tdHdr" style="width: 100px; text-align: right;">
                            Name:
                        </td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td>
                            <asp:DropDownList ID="txtAcctMgr" runat="server" Width="98%" DataSourceID="dsManagers"
                            ...
                            <asp:SqlDataSource ID="dsManagers" runat="server" ConnectionString="<%$ ConnectionStrings:IDENTIFYLEDB %>"
                                SelectCommand="select UserId, FirstName + ' ' + LastName[Name] from tbluser where UserTypeId in(1,2) union all select -1[Userid],'None'[Name] ORDER BY Name ">
                            </asp:SqlDataSource>
                        </td>
                    </tr>
                    <tr>
            ...
            <div id="btab1">
                <div>
                    ...
                    <div class="rndBoxBluefg">
                        <div id="divCampaignGrid" style="height: 350px; overflow-x: hidden; overflow-y: scroll;">
                        </div>
                    </div>
        ...
        <hr />
        <asp:Label ID="lblLastModified" runat="server" Style="float: left;" />
        <div style="text-align: right;">
            <button id="btnSaveAffiliate" class="jqSaveButton" onclick="return SaveAffiliate();">
                Save Affiliate
            </button>
        </div>
    </div>
    ...
    <div id="dialog-contact" title="Affiliate Contact">
        <table style="width: 100%;">
            ...
            <tr>
                ...
                <td>
                    <asp:TextBox ID="txtContactNotes" runat="server" Width="200px" TextMode="MultiLine"
                        Rows="2" />
                </td>
            </tr>
             <tr>
