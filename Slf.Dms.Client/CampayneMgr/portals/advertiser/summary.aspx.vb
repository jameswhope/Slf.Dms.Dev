...
Partial Class portals_advertiser_Summary
...
#Region "Methods"
    ...
    Private Sub GetSummaryRow()
        For i As Integer = 0 To gvTraffic.Columns.Count - 1
            gvTraffic.Columns(i).FooterText = ConfigurationManager.AppSettings("SummaryFooterText").ToString
        Next
        Using dv As DataView = dsTraffic.Select(DataSourceSelectArguments.Empty)
            Using dt As DataTable = dv.ToTable
                If dt.Rows.Count > 0 Then
                    gvTraffic.Columns(2).FooterText = FormatCurrency(dt.Compute("avg(priceReceived)", Nothing).ToString, 2)
                    gvTraffic.Columns(3).FooterText = dt.Compute("sum(clicks)", Nothing).ToString
                    gvTraffic.Columns(4).FooterText = dt.Compute("sum(conversions)", Nothing).ToString
                    gvTraffic.Columns(5).FooterText = FormatCurrency(dt.Compute("sum(revenue)", Nothing).ToString, 2)
                End If
            End Using
        End Using
...
#End Region 'Methods
    ...
    Protected Sub dsTraffic_Selecting(sender As Object, e As System.Web.UI.WebControls.SqlDataSourceSelectingEventArgs) Handles dsTraffic.Selecting
