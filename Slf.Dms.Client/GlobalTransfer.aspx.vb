...
#End Region
    ...
    Protected Sub GlobalTransfer_Load(sender As Object, e As System.EventArgs) Handles Me.Load

        LoadClients()

    End Sub

    Protected Sub ReadCookies() 'Get the cookie name the user entered
        lblFirstName.Text = URLDecode(Request.Cookies("firstname").Value)
        lblLastName.Text = URLDecode(Request.Cookies("lastname").Value)
        lblAddress.Text = URLDecode(Request.Cookies("address").Value)
        lblCity.Text = URLDecode(Request.Cookies("city").Value)
        lblState.Text = Request.Cookies("state").Value
        lblZipCode.Text = Request.Cookies("zipcode").Value
        lblEmails.Text = Request.Cookies("email").Value
        lblPhones.Text = URLDecode(Request.Cookies("phone").Value)
        lblLanguage.Text = Request.Cookies("clientid").Value
        lblClient.Text = Request.Cookies("clientid").Value
    ...
    Protected Sub LoadClients()
        Dim stp As String = "stp_GlobalGetOutstandingClients"
        Dim dt As DataTable = SqlHelper.GetDataTable(stp, CommandType.StoredProcedure)

        rpClients.DataSource = dt
        rpClients.DataBind()

        lblCount.InnerText = "(" + dt.Rows.Count.ToString + ") of 2073"
    End Sub
    ...
    Protected Sub lnkPrint_Click(sender As Object, e As System.EventArgs) Handles lnkPrint.Click
        ...
        Dim rDoc As GrapeCity.ActiveReports.Document.SectionDocument
    ...
    Protected Sub lnkEmail_Click(sender As Object, e As System.EventArgs) Handles lnkEmail.Click
        ReadCookies()
    ...
    Protected Sub lnkNote_Click(sender As Object, e As System.EventArgs) Handles lnkNote.Click
        Dim ClientId As Integer = lblClient.Text
        Dim Message As String = txtMessage.Text
        Dim UserId As Integer = 1265

        InsertNote(Message, UserId, ClientId)
