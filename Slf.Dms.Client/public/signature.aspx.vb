...
Partial Class public_signature
...
#Region "Fields"
    ...
    Private _CodebtorInitImgPath As String
        ...
        Private _IsHyperlinked As Boolean
...
#Region "Methods"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            'Added ccastelo 11/27/12 9:12am
            'checks for mobile devices then redirect to new page if device is mobile.

            'Added _IsHyperLinked at top of page as private variable.
            If Request.QueryString("hyp") = "" Then
                _IsHyperlinked = False
            Else
                _IsHyperlinked = CBool(Request.QueryString("hyp"))
            End If

            If isMobileDevice() AndAlso Not _IsHyperlinked Then
                Response.Redirect(String.Format("mobile_esign.aspx?sbId={0}", _SigningBatchID))
            End If
            'End

            hpMobile.HRef = String.Format("mobile_esign.aspx?sbId={0}&hyp=1", _SigningBatchID)
            ...
            If tblDoc.Rows.Count >= 1 Then
                If tblDoc.Rows(0)("completed") Is DBNull.Value Then
                    ...
                    For Each d As DataRow In tblDoc.Rows
                        ...
                        tbldocs.Add(d("displayname").ToString, ConfigurationManager.AppSettings("LeadDocumentsDir") & String.Format("temp\{0}.html", d("documentid").ToString))
        ...
        'Added ccastelo 11/27/2012 9:18am
        'checks for mobile devices
        Public Function isMobileDevice() As Boolean
            'get current user context
            Dim context As HttpContext = HttpContext.Current

            'check built in asp.net basic check
            If context.Request.Browser.IsMobileDevice Then
                Return True
            End If

            'check for HTTP_X_WAP_Profile Header
            If context.Request.ServerVariables("HTTP_X_WAP_PROFILE") <> Nothing Then
                Return True
            End If

            'check the header variable user agent for mobile variable
            Dim devices() As String = {"midp", "j2me", "avant", "docomo", "novarra", "palmos", "palmsource", "240x320", _
                                       "opwv", "chtml", "pda", "windows ce", "mmp/", "blackberry", "mib/", "symbian", "wireless", _
                                       "nokia", "hand", "mobi", "phone", "cdm", "up.b", "audio", "SIE-", "SEC-", "samsung", "HTC", _
                                       "mot-", "mitsu", "sagem", "sony", "alcatel", "lg", "eric", "vx", "NEC", "philips", "mmm", _
                                       "xx", "panasonic", "sharp", "wap", "sch", "rover", "pocket", "benq", "java", "pt", "pg", _
                                       "vox", "amoi", "bird", "compal", "kg", "voda", "sany", "kdd", "dbt", "sendo", "sgh", _
                                       "gradi", "jb", "dddi", "moto", "iphone"}
            For Each s As String In devices
