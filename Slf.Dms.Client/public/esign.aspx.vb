...
#Region "Fields"
    ...
    Public _documentNeedsInitials As Boolean
#End Region 'Fields
...
#Region "Properties"
    ...
    Public Property HFiles() As Dictionary(Of String, String)
        Get
...
#Region "Methods"
    ...
    Public Sub BuildDocumentList(ByVal hFiles As Dictionary(Of String, String), ByVal DebtorSigImgPath As String, ByVal CodebtorSigImgPath As String, ByVal DebtorInitImgPath As String, ByVal CodebtorInitImgPath As String)
        ...
        For Each doc As KeyValuePair(Of String, String) In hFiles
            Dim lbl As New System.Web.UI.WebControls.Label
            Dim docNode As New System.Web.UI.WebControls.TreeNode(doc.Key)
            ...
            Using sr As New StreamReader(doc.Value)
            ...
            _formHTML = _formHTML.Replace("<div style=""page-break-inside:avoid;", "<div runat=""server"" onfocus=""this.scrollTop=99999;"" style=""border:solid 1px black;padding:10px;page-break-inside:avoid;min-height:590px;height:100%;width:100%;overflow-x: hidden; overflow-y: scroll; position:relative;background-color:white;")
        ...
        If iTotalInits > 0 Then
            _documentNeedsInitials = True
        ...
        Else
            _documentNeedsInitials = False
            divInfo.InnerHtml = String.Format("You need to <a href=""#"" onclick=""jumpToFirst(1);"">sign in {0} location(s)</a>.", iTotalSigs)
        End If
    ...
    Protected Sub Page_LoadComplete(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.LoadComplete
        If Not IsPostBack Then
            Dim bJS As Boolean = False
            If Request.Browser.EcmaScriptVersion.Major > 0 Then
                bJS = True
            End If
            LexxSignHelper.Document_updateBrowserInfo(_SigningBatchID, Request.Browser.Browser, Request.Browser.Version, bJS)
        End If
    End Sub
    ...
    Private Sub emailStatusToSubmittedBy()
        Dim firmName As String = ""
        Dim submittedByEmail As String = ""
        Dim sqlInfo As New StringBuilder
        sqlInfo.Append("select distinct co.name,u.emailaddress from tblleaddocuments ld with(nolock) ")
        sqlInfo.Append("inner join tblleadapplicant la with(nolock) on la.leadapplicantid = ld.leadapplicantid ")
        sqlInfo.Append("inner join tblcompany co with(nolock) on co.companyid = la.companyid ")
        sqlInfo.Append("inner join tbluser u with(nolock) on u.userid = ld.submittedby ")
        sqlInfo.AppendFormat("where ld.signingbatchid = '{0}'", _SigningBatchID)
        Using dt As DataTable = SqlHelper.GetDataTable(sqlInfo.ToString, CommandType.Text)
            For Each row As DataRow In dt.Rows
                firmName = row("name").ToString
                submittedByEmail = row("emailaddress").ToString
                Exit For
            Next
        End Using
        ...
        'message.From = New MailAddress("info@lawfirmcs.com", firmName)
        'message.To.Add(New MailAddress(submittedByEmail))
        'message.Subject = String.Format("The documents (between {0} and {1}) are Signed and Filed!", firmName, lblLeadName.Text)
        'message.Body = String.Format("The documents (between {0} and {1}) are Signed and Filed!", firmName, lblLeadName.Text)
        ...
        'mailMsg.To.Add(New System.Net.Mail.MailAddress(submittedByEmail))
        'mailMsg.Subject = String.Format("The documents (between {0} and {1}) are Signed and Filed!", firmName, lblLeadName.Text)
    ...
    Protected Sub lnkSign_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkSign.Click
        ...
        _DocumentInfo = Document_getDocumentID(_SigningBatchID)
        ...
        If Integer.Parse(hdnSignCount.Value) = 0 AndAlso Integer.Parse(hdnInitCount.Value) = 0 Then
            ...
            For Each hf As KeyValuePair(Of String, String) In HFiles
                Try
                    'build and save signed doc
                    Dim pdfName As String = hf.Value.Replace(".html", ".pdf")
                    ...
                    If File.Exists(signedDocPath) Then
                        'Using inFile As New System.IO.FileStream(signedDocPath, IO.FileMode.Open, IO.FileAccess.Read)
                        '    Dim fileExt As String() = hf.Value.Split("/")
                        '    Dim filenames As String() = fileExt(fileExt.Length - 1).Split(".")
                        '    Dim filename As String = filenames(0)
                        '    AzureStorageHelper.ExportLeadDocumentTemp(inFile, filename)
                        'End Using
                        Using inFile As New System.IO.FileStream(signedDocPath, IO.FileMode.Open, IO.FileAccess.Read)
                            Dim fileExt As String() = hf.Value.Split("\")
                            Dim filenames As String() = fileExt(fileExt.Length - 1).Split(".")
                            Dim filename As String = filenames(0) + ".pdf"
                            AzureStorageHelper.ExportLeadDocumentTemp(inFile, filename)
                        End Using
                    End If
                    signedFiles.Add(hf.Key, signedDocPath)
                Catch ex As Exception
            ...
            Document_updateCompletedDateBySigningBatchID(_SigningBatchID, GetClientIP())

            'email signed doc to lead
            Document_emailSignedDocuments(_DocumentInfo.DocumentLeadEmail, lblLeadName.Text, _SigningBatchID, signedFiles)

            Try
                Dim leadAppID As String = DataHelper.FieldTop1("tblLeadDocuments", "LeadApplicantID", "LeadDocumentID = " & _DocumentInfo.LeadDocumentID)
                Dim repID As String = DataHelper.FieldTop1("tblleadapplicant", "repid", "LeadApplicantID = " & leadAppID)
                Dim repEmail As String = DataHelper.FieldTop1("tbluser", "emailaddress", "UserID = " & repID)

                'email signed doc to Representative / replace cholt with representative variable
                Document_emailSignedDocumentsToRep(repEmail, lblLeadName.Text, _SigningBatchID, signedFiles)
            Catch ex As Exception
                'do nothing
            End Try
    ...
    Private Shared Function BuildDocumentLinkPath(ByVal typeOfPerson As enumSignaturePerson,
        ByVal typeOfSignature As enumSignatureType,
        ByVal iDocCnt As Integer,
    ...
    Private Shared Function BuildDocumentTreeNodeLinkPath(ByVal typeOfPerson As enumSignaturePerson,
        ByVal typeOfSignature As enumSignatureType,
        ByVal iDocCnt As Integer,
        ByVal ipage As Integer,
        ByVal numAppSigs As Integer) As String
    ...
    Private Function Build_swapTextWithImage(ByVal typeOfPerson As enumSignaturePerson,
        ByVal typeOfSignature As enumSignatureType,
        ByVal signatureImgPath As String,
        ByVal iDocCnt As Integer,
        ByVal appliedSignatures As Dictionary(Of String, Dictionary(Of String, List(Of String))),
    ...
    Private Sub Document_emailSignedDocument(ByVal SendTo As String, ByVal signedPath As String)
        Dim mailMsg As New System.Net.Mail.MailMessage()
        mailMsg.From = New System.Net.Mail.MailAddress("noreply@lawfirmsd.com", "Law Firm Compliance")
        mailMsg.To.Add(New System.Net.Mail.MailAddress(SendTo))
        mailMsg.Subject = String.Format("The {0} (between Law Firm Compliance and {1}) is Signed and Filed!", hDocName.InnerText, lblLeadName.Text)

        Dim msgBody As New StringBuilder
        msgBody.Append("Your signed document is attached.")
        mailMsg.Body = msgBody.ToString

        Dim att As New System.Net.Mail.Attachment(signedPath)
        att.Name = hDocName.InnerText.Replace(" ", "_") & ".pdf"
    ...
    Private Sub GetDocument(ByVal docId As String)
        Dim path As String = ConfigurationManager.AppSettings("LeadDocumentsTemp") & String.Format("{0}.pdf", docId)
    ...
    Private Function GetDocumentsBySigningBatchID(ByVal signingBatchId As String) As Dictionary(Of String, String)
        Dim path As String = ConfigurationManager.AppSettings("LeadDocumentsTemp") & String.Format("{0}.html", signingBatchId)
        Dim tblDoc As DataTable = SqlHelper.GetDataTable(String.Format("stp_esign_getDocuments '{0}'", signingBatchId))

        Dim tbldocs As New Dictionary(Of String, String)
        ...
        If tblDoc.Rows.Count >= 1 Then
            If tblDoc.Rows(0)("completed") Is DBNull.Value Then
                ...
                For Each d As DataRow In tblDoc.Rows
                    ...
                    tbldocs.Add(d("displayname").ToString, ConfigurationManager.AppSettings("LeadDocumentsTemp") & String.Format("{0}.html", d("documentid").ToString))
