<%@ Page Language="VB" AutoEventWireup="false" CodeFile="attachaccountdoc.aspx.vb" Inherits="util_pop_attachaccountdoc" %>
...
<body style="background-image: url(<%= ResolveUrl("~/images/back.bmp") %>); background-position: left top;
    ...
    <asp:SqlDataSource ID="ds_DocTypes" runat="server" ConnectionString="<%$ AppSettings:connectionstring %>"
        SelectCommand="select distinct typeid, displayname from tbldocumenttype where not typeid like '%scan%' and docfolder = 'CreditorDocs' order by displayname"
